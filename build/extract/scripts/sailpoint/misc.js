...
/** Trim function **/
// TODO: Do we need this? Pretty sure all our browsers support trim by now.
...
function setBrowserTabId() {
    if(Ext.get("editForm:tabId") != null){
        if(window.name == "") {
            if(Ext.getDom("editForm:tabId").value != ""){ // this probably shouldn't happen
                window.name = Ext.getDom("editForm:tabId").value;
            }
            else {
                window.name = generateTabId();
                Ext.getDom("editForm:tabId").value = window.name;
            }
        }
        // Either a blank value, or we're on a different tab than we should be.
        else if (Ext.getDom("editForm:tabId").value != window.name) {
            Ext.getDom("editForm:tabId").value = window.name;
        }
    }
}
...
var ieJavascriptFixingObserver = {
    ...
    onShow: function() {
        ...
        var jsDivs = Ext.select('div[class=ieJavascriptFixDiv]');
...
function showHideForRadio(targetElement, sourceElement, displayOnValue) {
    ...
    var i=0;
    while(i < endIndex) {
        if(showThisElement && !element.isVisible()) {
        ...
        } else if(!showThisElement && element.isVisible()){
...
function resizeTables(contentElId) {
    if(Ext.isIE7) {
        ...
        var tables = Ext.DomQuery.select('table[class*=width100]', Ext.getDom(contentElId));
        if (tables != null) {
...
function getInputDateValue(id) {
  var day = Ext.getDom(id+".day").value;
  var month = Ext.getDom(id+".month").value - 1;
  var year = Ext.getDom(id+".year").value;
...
function toggleDisplay(element, condition) {
  if(element) {
      Ext.getDom(element).style.display = condition ? 'none' : '';
  }
}
...
function enableMultiSelect(selectName, suggestName, disabled) {
    ...
    var selectElt = Ext.getDom(selectName);
    var suggestElt = Ext.getDom(suggestName);
    var selectBox = Ext.getDom(suggestName + '.selectBox');
    var addBtn = Ext.getDom(suggestName + '.addButton');
...
function hashToMapString(hash) {
    ...
    var sep = "";
      ...
      Ext.Object.each(hash, function(key, val) {
          mapString += sep + key + "=" + val;
         sep = ",";
      });
...
function countSelected(container, checkboxIds, displayPanel)
{
    ...
    if(null != container)
    {
        ...
        var checkboxes = Ext.Array.toArray(checks);
...
function getElementsByClassAndSuffix(className, idSuffix) {
    ...
    var resultArray = [];
    Ext.each(eltByClassArray, function(element) {
        if ((element.id.indexOf(idSuffix) + idSuffix.length) === element.id.length) {
...
Ext.ns('SailPoint');
SailPoint.VirtualRadioButton = function(containerId, radioNamePrefix) {
...
//        }
    ...
    updateRadios: function() {
        ...
        Ext.each(radios, function(radio, idx) {
    ...
    getChangedValues: function() {
        ...
        var newHash = {};
        Ext.Object.each(this.changedValues, function(key, val) {
            var safeKey = this.stripPrefix(key);
...
SailPoint.VirtualCheckbox = function() {};
...
SailPoint.VirtualCheckbox.prototype = {
    ...
    toggleSelectAll: function(selectAllCheckbox) {
        ...
        if (null != selectAllCheckbox) {
            ...
            Ext.each(checkArray, function(checkboxElement, idx) {
    ...
    displaySelectedCount: function(selectedCountElt, divClass) {
        var textElement = Ext.get(selectedCountElt);
        if (1 == this.count) {
            textElement.dom.innerHTML =
    ...
    hideSelectedCount: function(selectedCountElt) {
        var textElement = Ext.getDom(selectedCountElt);
...
Ext.define('SailPoint.SubmitOnEnter', {
   ...
   inputTextFieldsCheck : function(node) {
       var root = node ? Ext.getDom(node) : document;
       var inputTextFields = Ext.DomQuery.select('input', root);
       if ( inputTextFields) {
           for (var i = 0; i < inputTextFields.length; i++) {
               var inputField =  inputTextFields[i];
               if (inputField.getAttribute("type") === 'text') {
                   var extInputField = Ext.get(inputField);
                   var parentSuggest = extInputField.up('div[id*="Suggest"]');
...
SailPoint.TrueFalseAJAXChecker = function() {};
SailPoint.TrueFalseAJAXChecker.prototype = {
    ...
    isResponseTrue: function() {
        if (!this.ajaxRequestExecuted) {
            ...
            var ajaxReq = Ext.Ajax.request(
                {
                    url: this.ajaxUrl,
                    method: 'post',
                    params: this.requestParams.toQueryString(),
                    callback: this.onCompleteCallback.bind(this),
                    asynchronous: false
                });
        }
...
SailPoint.NavigationHistory = {
    ...
    getCancelFunction: function(objectId) {
        return function(win) {
            if (null != objectId) {
                Ext.getDom('navigationForm:objectId').value = objectId;
            }
...
function makeEscapedQueryParam(queryParam) {
    if (queryParam) {
        return queryParam.
                   replace(/%/g, '%25').  // do this first or encoding will be corrupted
                   replace(/&/g, '%26').
                   replace(/ /g, '%20').
                   replace(/\+/g, '%2B').
                   replace(/#/g, '%23').
...
function shortenElementLabels(elementLabels, maxLength) {
    ...
    for (i = 0; i < elementLabels.length; ++i) {
        ...
        if (originalNodeText.length > MAX_LABEL_LENGTH) {
            ...
            hiddenFloatie = Ext.getDom(hiddenFloatieId);
...
function viewAccountGroup(appName, groupAttr, group, id, refererType, refererId, parentId, certificationItemId) {
    ...
    if (id) {
        ...
        params = {
            ...
            groupName: group,
            certificationItemId: certificationItemId,
    ...
    if (refererType) {
        params.refererType = refererType;
    }
    if (refererId) {
        params.refererId = refererId;
    }
    if (parentId) {
    ...
    var counterStore = SailPoint.Store.createStore({
        ...
        fields: ['id',
            ...
            "accessGridMetaData",
            "classificationCount",
            "classificationGridMetaData",
            "isIncludeClassifications",
            "isCamBased",
            "camRolesMetaData",
            "camScopesMetaData",
            "camServicesMetaData",
            "camResourcesMetaData",
            "assocRolesCount",
            "assocRolesGridMetaData",
            "displayName"
        ],
        listeners: {
...
// shows link details on identity or certification pages
 function showLinkDetails(targetId, linkId, certItemId, showNonEntitlementsOnly, sourceLink, refererType, refererId){

    var containingDiv = Ext.get("linkDetails_" + targetId);
    ...
    var containingTR = Ext.get("linkDetailsTR_" + targetId);
    if (containingTR && (containingTR.isVisible() === containingDiv.isVisible())){
        elemsToShowHide.push(containingTR);
    }

    // update the content area
    if (!containingDiv.isVisible()){
        ...
        if (!div.hasCls('loaded')){
            ...
            if (refererType) {
                url += '&refererType=' + refererType + '&refererId=' + refererId;
            }
...
function addDescriptionTooltipsFn(rootNode) {
  ...
  if (null != values) {
    for (i = 0; i < values.length; i++) {
      ...
      if(id) {
        description = Ext.get('description_'+id);
      }
      ...
      if (description) {
          var text = description.dom ? description.dom.firstElementChild.innerHTML : description.firstElementChild.innerHTML
...
SailPoint.LoadingSpinner = function() {
...
SailPoint.Log = {
    ...
    _getLogElement: function() {
        var elt = Ext.get('SailPointLogDiv');
        if (null == elt) {
...
function resizeIdentityDetailsContainer(element) {
    ...
    elements.forEach(function resizeAll(element) {
