...
Ext.define('SailPoint.grid.CheckboxSelectionModel' , {
    ...
    */ 
    ...
    /**
     * In the event you would like to display text in the header. Usually used in conjunction with the
     * property 'showHeaderCheckbox' set to false.
     */
    headerText: null,
    /**
     * When using headerText, set this integer property also. The defaultWidth of a column is 100 pixels which may be
     * too large for displaying your headerText. Attempts at using a flex property were unsuccessful, ExtJS still
     * calculated a width value too large.
     */
    headerTextWidth: null,
    ...
    getSelectedNames : function() {
        ...
        if (this.selected.localSelections) {
            this.selected.localSelections.each(function(model) {
                // if the 'name' field is not defined for any row,
                // empty the array and exit the loop.
                if (!model.raw.name) {
                    names = [];
                    return false;
                } else {
                    names.push(model.raw.name);
                }
            });
        }
        return names;
    ...
    */
    ...
    isStoreFiltered: function() {
        var hasFilters = false,
        extraParams = this.store.getProxy().extraParams;
        Ext.iterate(extraParams, function(paramName) {
            if(paramName !== 'limit' && paramName !== 'start') {
                if(extraParams[paramName]) {
                    hasFilters = true;
                }
            }
        });
    ...
    getHeaderConfig: function() {
        ...
        if (this.headerText) {
            conf.text = this.headerText;
        }
        if (this.headerTextWidth) {
            conf.width = this.headerTextWidth;
        }
