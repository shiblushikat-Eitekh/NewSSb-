...
Ext.define('SailPoint', {
    statics : {
        ...
        sanitizeHtml : function(text) {
            ...
            // encode < character to avoid XSS issues
        ...
        /* There is so much bloody padding that the window inner height is very often less than 768, we always
         * need to support a more minimal minimum, say 300.
         */
        minSupportedHeight : 300,

        /**
         * Navigation to the home page.
         */
        NAV_PAGE_HOME : 'viewHome'
    }
});


// Use this flag to disable the ext session dialogs.
SailPoint.disableExtSessionDialogs = false;
...
SailPoint.resetTimeout = function() {
    // Disable the session timeout dialogs
    if (SailPoint.disableExtSessionDialogs) {
        return;
    }
    // first clear out any existing session timeout monitor
    window.clearTimeout(SailPoint.TIMEOUT_ID);
    window.clearTimeout(SailPoint.EARLY_WARNING_TIMEOUT_ID);
    var miliWait = SailPoint.getSessionTimeout() + 30000;
    SailPoint.TIMEOUT_ID = window.setTimeout("showTimeoutMsg()", miliWait);
    // Show an early warning message five minutes before the timeout message.
    // If timeout is less than five minutes don't bother
    var earlyWarningTimeout = miliWait - 300000;
    if(earlyWarningTimeout > 0) {
        SailPoint.EARLY_WARNING_TIMEOUT_ID = window.setTimeout("addEarlyWarningMsg()", earlyWarningTimeout);
    }
};

/**
 * Object to hold URLs that are used for Ajax polling. Key will be the URL and value will be boolean true.
 */
SailPoint.POLLING_URLS = {};

/**
 * Add a URL to the set of polling URLs, to be skipped for automatic session reset
 * @param {String} url URL to add
 */
SailPoint.addPollingUrl = function(url) {
    SailPoint.POLLING_URLS[url] = true;
};

/**
 * @param {String} url URL to verify
 * @returns {boolean} True if URL is in the set of polling URLs, otherwise false.
 */
SailPoint.isPollingUrl = function(url) {
    return !!SailPoint.POLLING_URLS[url];
};
...
SailPoint.FATAL_ERR_ALERT = function(msg) {
    ...
    // sanitize the alert message to avid XSS issues
    Ext.Msg.show({
...
SailPoint.confirm = function(content, options) {
    ...
    if (initialLoad) {
        ...
        SailPoint.Dialog.setTitle(Ext.String.htmlEncode(title));
...
/* If We are not including Prototype on the page we will not be using Prototype for XHR,
 * so guard against the undefined reference. */
if(window.Ajax) {
    /**
