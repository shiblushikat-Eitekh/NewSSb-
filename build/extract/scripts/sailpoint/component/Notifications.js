...
Ext.define('sailpoint.notification.ConfigBase', {
    ...
    statics : {
        copyBaseProperties : function (from, to) {
            ...
            to.recipientsRuleName = from.recipientsRuleName;
        }
...
Ext.define('sailpoint.notification.NotificationBase', {
    ...
    , initDom : function () {
        this.emailTemplateSelect = Ext.getDom("emailTemplateSelect");
        this.additionalFieldset = Ext.getDom("additionalFieldset");
        this.additionalContainer = Ext.get("additionalContainer");
        this.additionalContainer.setVisibilityMode(Ext.Element.DISPLAY);
        this.additionalRecipientsCheckbox = Ext.getDom("additionalRecipientsCheckbox");
        this.additionalRecipientsCheckbox.onclick = this.showHideAdditionalRecipients.bind(this);
        this.recipientsRuleSelect = Ext.getDom("recipientsRuleSelect");
        ...
        this.additionalRecipientsErrorLabel = Ext.get("additionalRecipientsErrorLabel");
        this.actionButton = Ext.getDom("actionButton");
        this.cancelButton = Ext.getDom("cancelButton");
    }
    , copyBaseValuesFromInput : function (config) {
        ...
        if (config.additionalEmailRecipientsPresent) {
            ...
            config.recipientsRuleName = SailPoint.Utils.getValueFromSelect(this.recipientsRuleSelect, null);
        } else {
            config.additionalRecipients =  [];
            config.recipientsRuleName = null;
        }
    }
    , loadValuesFromInput : function () {
        ...
        if (this.input.additionalEmailRecipientsPresent) {
            ...
            SailPoint.Utils.setValueInSelect(this.recipientsRuleSelect, this.input.recipientsRuleName);
    ...
    , disable : function () {
        this.emailTemplateSelect.disabled = true;
        this.additionalRecipientsCheckbox.disabled = true;
        this.additionalRecipientsMultiSuggest.disabled = true;
        this.recipientsRuleSelect.disabled = true;
        this.actionButton.disabled = true;
        this.cancelButton.disabled = true;
    }
    , enable : function () {
        this.emailTemplateSelect.disabled = false;
        this.additionalRecipientsCheckbox.disabled = false;
        this.additionalRecipientsMultiSuggest.disabled = false;
        this.recipientsRuleSelect.disabled = false;
    ...
    , validate : function () {
        ...
        if (this.additionalRecipientsCheckbox.checked && this.additionalRecipientsMultiSuggest.selectedStore.getCount() < 1 && this.recipientsRuleSelect.selectedIndex < 1) {
            ...
            this.additionalRecipientsErrorLabel.dom.innerHTML = "* Please select a recipient or recipient rule";
        }
        return valid;
    }
...
Ext.define('sailpoint.notification.Notifications', {
    ...
    , initializeNotificationsConfig : function () {
        ...
        if (jsonString == null || jsonString.trim().length == 0) {
            ...
            this.notificationsConfig.configs =  [];
        } else {
    ...
    , initializeStartAndEndDatesFromHiddenElements : function () {
        if (this.runNowElement && Ext.getDom(this.runNowElement.id).checked) {
            this.notificationsConfig.startDate = sailpoint.util.Day.today().getTime();
        } else {
            this.notificationsConfig.startDate = parseFloat(Ext.getDom(this.hiddenStartDateElement.id).value);
        }
        this.notificationsConfig.endDate = parseFloat(Ext.getDom(this.hiddenEndDateElement.id).value);
    }
    ...
    , createOrderedConfigs : function () {
        var ordered =  [],
            configValues = Ext.Object.getValues(this.configsHash);
    ...
    , onEditReminderLoad : function (sequence) {
        ...
        if (!this.isEditAllowed(reminderConfig)) {
            this.currentNotification.disabled = true;
        }
    ...
    , onConfirmDeleteClick : function (sequence, val) {
        if (val == "yes") {
            delete this.configsHash[sequence];
            this.createOrderedConfigs();
        }
    }
