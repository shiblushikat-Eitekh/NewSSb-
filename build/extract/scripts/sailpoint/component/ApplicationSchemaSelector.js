Ext.define('sailpoint.component.ApplicationSchemaSelector', {
    ...
    constructor : function(config) {
        ...
        if (this.getSelectedSchemasObj().length == 0) {
            Ext.getDom(this.selectGroupsCheckboxId).checked = '';
        } else {
            Ext.getDom(this.selectGroupsCheckboxId).checked = 'checked';
    ...
    clearAllSchemas: function() {
        Ext.getDom(this.hiddenSchemasId).value = Ext.JSON.encode([]);
        this.updateSchemasTable();
    },
    ...
    clearSchema: function() {
        ...
        Ext.getDom(this.hiddenSchemasId).value = Ext.JSON.encode(selectedSchemasObj);
        this.updateSchemasTable();
    },
        ...
        this.schemaSuggest = SailPoint.SuggestFactory.createSuggest(
            ...
            '#{msgs.task_in_account_group_aggregation_add_group}',
            {
                ...
                tpl: new Ext.XTemplate(
                    ...
                    '<div class="sectionHeader">{appName:htmlEncode}</div>',
                    '<div class="indentedColumn">{schemaName:htmlEncode}</div>',
                    '</div>',
                    '</tpl>'
                ),
    ...
    getSelectedSchemasObj: function () {
        ...
        if (!Ext.getDom(this.hiddenSchemasId).value || Ext.getDom(this.hiddenSchemasId).value.length == 0) {
            selectedSchemasObj = [];
        } else {
            selectedSchemasObj = Ext.JSON.decode(Ext.getDom(this.hiddenSchemasId).value);
        }
    ...
    updateSchemasTable: function() {
        var schemas = Ext.JSON.decode(Ext.getDom(this.hiddenSchemasId).value);
    ...
    removeSchema: function(applicationId, schemaId) {
        ...
        Ext.getDom(this.hiddenSchemasId).value = Ext.JSON.encode(schemasToKeep);
        this.updateSchemasTable();
    },
    ...
    createSelectedSchemasTemplate: function() {
        return new Ext.XTemplate(
            ...
            '<div style="height:116px; padding:0px; vertical-align: center"></div>',
            ...
            '<li style="display:inline; list-style:none; margin-left:0px"><div class="sectionHeader">#{msgs.application}: {name:htmlEncode}</div></li>',
            '<tpl for="schemas">',
            ...
            'width="12" height="12"/>',
            '</span>',
            '<span style="padding:3px">{name:htmlEncode}</span>',
            '</div></a>',
            '<tpl else>',
            '<div style="padding:3px">{name:htmlEncode}</div>',
            '</tpl>',
            '<//li>',
            '</tpl>',
            '</tpl>',
