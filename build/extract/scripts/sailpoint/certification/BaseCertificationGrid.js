...
Ext.define('SailPoint.certification.BaseCertificationGrid', {
    ...
    constructor : function(config) {
        ...
        if (config.editable) {
            sm = new SailPoint.grid.CheckboxSelectionModel({selectMessageBox: Ext.get('selectedCount')});
        }
        ...
        if (config.gridMetaData){
            Ext.each(config.gridMetaData.columns, function(column){
    ...
    updateButtonGroups : function(){
        if (this.buttonGroups){
            Ext.each(this.buttonGroups, function(group){
...
SailPoint.certification.BaseCertificationGrid.renderRoleDetails = function(value, metadata, record, rowIdx, colIdx, store) {
    try{
        ...
        // IIQFW-2: Encode role name to avoid HTML injection
        var roleName = Ext.String.htmlEncode(value);
        var roleDetails = record.get('IIQ_roleDetails');
        ...
        if (roleDetails.roleId) {
            if (roleDetails.iiqElevatedAccess) {
                roleName += "&nbsp;<img class=\"iiqElevatedAccessIcon iiq-elevated-access-icon-link\" " +
...
SailPoint.certification.BaseCertificationGrid.renderAccountAttribute = function(value, metadata, record, rowIdx, colIdx, store) {
    ...
    try{
        ...
        if (entitlement){
            html = SailPoint.certification.BaseCertificationGrid.createEntitlementHtml(entitlement.name, entitlement.description, null, false, record.internalId, record)
        }
    ...
    try {
        ...
        for(var i=0;i<entitlements.length;i++){
            ...
            html += SailPoint.certification.BaseCertificationGrid.createEntitlementValuesString(entitlement,
                    value.application, value.isNew, record.internalId, record);
        }
    } catch(err){
    ...
    try{
        ...
        if (value.permissions && value.permissions.length > 0){
            for(var p=0;p<value.permissions.length;p++){
                ...
                html += SailPoint.certification.BaseCertificationGrid.createEntitlementHtml(permission.name, permission.description, null, value.isNew, record.internalId, record);
                html += "</div>";
                html += "<div style='margin-left:15px'>";
                html += SailPoint.certification.BaseCertificationGrid.createEntitlementValuesString(permission, value.application, value.isNew, record.internalId, record);
                html += "</div>";
            }
        }
        ...
        if (value.attributes && value.attributes.length > 0){
            for(var a=0;a<value.attributes.length;a++){
                ...
                html += SailPoint.certification.BaseCertificationGrid.createEntitlementHtml(attribute.name, attribute.description, null, value.isNew, record.internalId, record);
                html += "</div>";
                html += "<div style='margin-left:15px'>";
                html += SailPoint.certification.BaseCertificationGrid.createEntitlementValuesString(attribute, value.application, value.isNew, record.internalId, record);
                html += "</div>";
            }
        }
 ...
 */
SailPoint.certification.BaseCertificationGrid.createEntitlementValuesString = function(entitlement, application, isNew, certificationItemId, record) {
    var html = "";
    for(var v=0;v<entitlement.values.length;v++){
        ...
        html += SailPoint.certification.BaseCertificationGrid.createEntitlementHtml(entitlementValue.displayValue,
                entitlementValue.description, groupInfo, isNew, certificationItemId, record);
 ...
 * @param groupInfo
 */
SailPoint.certification.BaseCertificationGrid.createEntitlementHtml = function(name, description, groupInfo, isNew, certificationItemId, record) {
    ...
    var iiqElevatedAccess;
    // Only fields with descriptions should display elevated access icons
    if (description != null && record != null) {
        iiqElevatedAccess = record.get('iiqElevatedAccess');
    }
    html += SailPoint.certification.BaseCertificationGrid.createEntitlementSpan(name, groupInfo, certificationItemId, iiqElevatedAccess);
    ...
    html += SailPoint.certification.BaseCertificationGrid.createEntitlementSpan(description ? description : name, groupInfo, certificationItemId, iiqElevatedAccess);
 ...
 */
SailPoint.certification.BaseCertificationGrid.createEntitlementSpan = function(text, groupInfo, certificationItemId, iiqElevatedAccess){

    // IIQFW-2: Encode entitlement text to avoid HTML injection
    ...
    if (groupInfo){
        var onclick = "viewAccountGroup('"+Ext.String.escape(groupInfo.app)+"','"+Ext.String.escape(groupInfo.attr)+"','"
                +Ext.String.escape(groupInfo.value)+"', undefined, 'certificationItem', '" + Ext.String.escape(certificationItemId) + "')";
        if (iiqElevatedAccess === true) {
            text += "&nbsp;<img class=\"iiqElevatedAccessIcon iiq-elevated-access-icon-link\" " +
                "title=\"#{msgs.role_is_iiq_elevated_access_label}\" " +
                "alt=\"#{msgs.role_is_iiq_elevated_access_label}\"/>";
        }
        ...
        if (iiqElevatedAccess === true) {
            text += "&nbsp;<img class=\"iiq-elevated-access-icon iiq-elevated-access-icon-image\" " +
                "title=\"#{msgs.role_is_iiq_elevated_access_label}\" " +
                "alt=\"#{msgs.role_is_iiq_elevated_access_label}\"/>";
        }
...
SailPoint.certification.BaseCertificationGrid.renderStatus = function(value, p, r) {
  if(value=='#{msgs.cert_item_stat_complete}') {
...
{
  try {
    Ext.getDom('editForm:selectedWorkItemId').value = id;
    Ext.getDom('editForm:viewWorkItemButton').click();
  } catch(e) {}
};
