...
Ext.define('SailPoint.certification.ViolationRemediationDialog', {
    ...
    reset : function() {
        ...
        if (Ext.fly('violationRemediationDialogName')) {
            Ext.getDom('violationRemediationDialogName').innerHTML = "";
            Ext.getDom('violationRemediationDialogDesc').innerHTML = "";
            Ext.getDom('EntitlementSodViolationTableDiv').innerHTML = "";
            Ext.getDom('violationSummaryContent').innerHTML = "";
            Ext.getDom('violationSelectionsContent').innerHTML = "";
        }

        // Remove any old rows in the role SOD selection tables
        if (Ext.getDom('roleSODDiv')) {
            Ext.each(Ext.query('.roleRemedRow', Ext.getDom('roleSODDiv')), function(element) {
                if (element)
        ...
        if (Ext.fly('violationDetailsDelegationReviewMsg')) {
            ...
            Ext.getDom('violationDetailsDelegationReviewMsg').innerHTML = '';
        }

        if (Ext.fly('SODViolationRemediationDialogAdvice')) {
            Ext.getDom('SODViolationRemediationDialogAdvice').innerHTML = '';
            ...
            success: function(response) {
                ...
                if (respObj.remediationDetails && respObj.remediationDetails.length > 0) {
                    ...
                    this.remediationEditor.showRevocationDetailsPanel(respObj.remediationDetails,
                            respObj.useManagedAttributesForRemediation, this.isReadOnly, this.certificationItem);
                }
                ...
                if (respObj.remediationAction == "SendProvisionRequest"){
                    ...
                    Ext.getDom('violationProvisioningMessageContent').innerHTML = provMsg;
    ...
    populateSummary : function(summary) {
        var comments = this.decision.comments ? this.decision.comments : summary.comments;
        ...
        if (summary.owner) {
            this.populateAssignmentField(summary.owner);
        }

        //this.populateAssignmentField(owner);
        if (summary) {
            this.populateAssignmentOptions(summary.assignmentOptions);
        }

        if (this.decision.requiresDelegationReview == true) {
            var delegationOwner = summary.delegation.owner;
            ...
            Ext.getDom('violationDelegationReviewMsg').innerHTML = reviewMsg;
            ...
            var detailMsg = Ext.String.format("#{msgs.cert_decision_violation_remediation_ok}",
                delegationOwner.displayName ? delegationOwner.displayName :
                    delegationOwner.name);
            Ext.getDom('violationDetailsDelegationReviewMsg').innerHTML = detailMsg;
        ...
        var assignmentCmp = Ext.getCmp('violationRemediationAssignee');
        if (!this.isReadOnly && !summary.enableOverrideDefaultRemediator && summary.defaultRemediator) {
            this.hideField(assignmentCmp);
        } else {
            this.showField(assignmentCmp);
        }
    ...
    populateSelectedRoles : function() {
        ...
        Ext.each(this.decision.revokedRolesDisplayableNames, function(role) {
            html += "<li>" + role + "</li>";
        });

        html += "</ul>";
        Ext.getDom('violationSelectionsContent').innerHTML = html;
    },
    ...
    handleRemediationAdvice : function(queryResponse) {
        try {
            ...
            Ext.getDom('violationRemediationDialogName').innerHTML = queryResponse.advice.violationConstraint;
            Ext.getDom('violationRemediationDialogDesc').innerHTML = queryResponse.advice.violationSummary;
            ...
            if (queryResponse.advice.remediationAdvice && queryResponse.advice.remediationAdvice != '') {
                Ext.getDom('SODViolationRemediationDialogAdvice').innerHTML = queryResponse.advice.remediationAdvice;
                ...
                if (requiresRoleRemediation || requiresEntitlementRemediation) {
                    ...
                    Ext.getDom('violationRemediationAdvicePanel').innerHTML = queryResponse.advice.remediationAdvice;
            ...
            if (queryResponse.summary) {
                this.populateSummary(queryResponse.summary);
            }
            ...
            if (requiresRoleRemediation) {
                ...
                Ext.getDom('violationSummaryContent').innerHTML = violationSummaryHtml;
    ...
    buildRoleTable : function(roles, elementId) {
        ...
        for (var i = 0; i < roles.length; i++) {
            ...
            var displayableName = Ext.String.htmlEncode(role.displayableName);
            if(role.iiqElevatedAccess){
                displayableName += '<img class="iiq-elevated-access-icon iiq-elevated-access-icon-image" ' +
                    'title="#{msgs.role_is_iiq_elevated_access_label}" ' +
                    'style="margin:0 2px 0 2px" ' +
                    'height="14px" width="14px" />';
            }
            var row = Ext.String.format(rowTpl, cmpId, isSelected === true ? "checked='true'" : '',
                    role.id, displayableName, role.description, itemStatus != null ? "style='display:none'" : "",
