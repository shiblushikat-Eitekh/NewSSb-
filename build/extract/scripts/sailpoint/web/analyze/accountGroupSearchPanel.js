...
Ext.define('SailPoint.AccountGroup.Search', {
    statics : {
        getAccountGroupSearchPanel : function(config) {
            ...
            var searchContents = Ext.create('SailPoint.TabContentPanel', {
                ...
                dockedItems: [{
                    ...
                    items: [{
                        id: 'advancedAccountGroupSearchNavBtn',
                        text: '#{msgs.advanced_search}',
                        scale : 'medium',
                        handler: SailPoint.AccountGroup.Search.displayAdvancedSearch,
                        cls: 'x-btn-text-icon'
                    }]
                }],
                bbar: [{
            ...
            var advancedSearchContents = Ext.create('Ext.panel.Panel', {
                id: 'advancedAccountGroupSearchContents',
                layout: 'fit',
                contentEl: 'advancedAccountGroupSearchContentsDiv',
                border: false,
                autoScroll: true,
                dockedItems: [{
                    xtype: 'toolbar',
                    ...
                    cls: 'advancedSearchToolbar',
                    items: [{
                        id: 'accountGroupSearchNavBtn',
                        text: '#{msgs.account_group_entitlement_search}',
                        scale : 'medium',
                        handler: SailPoint.AccountGroup.Search.displaySearchContents,
                        cls: 'x-btn-text-icon'
                    }]
                }],
                bbar: [{
                    id: 'preAdvancedAccountGroupSearchBtn',
                    text: '#{msgs.button_run_search}',
                    cls: 'primaryBtn',
                    handler: function() {
                        var searchPanel = Ext.getCmp('accountGroupSearchPanel');
                        searchPanel.validationErrors = SailPoint.Analyze.validateSearch('advancedAccountGroupSearchForm', 'advancedAccountGroup');
                    }
                }, {
                    id: 'advancedAccountGroupClearBtn',
                    text: '#{msgs.button_clear_search}',
                    handler: function() {
                        Ext.getDom('advancedAccountGroupSearchForm:resetBtn').click()
                    }
                }],
                loader: {
                    url: SailPoint.getRelativeUrl('/analyze/advanced/advancedSearchContents.jsf'),
                    params: {searchType: 'AdvancedAccountGroup'},
                    discardUrl: false,
                    callback: SailPoint.AccountGroup.Search.finishAdvancedSearchInit,
                    nocache: false,
                    text: '#{msgs.loading_data}',
                    timeout: 30,
                    scripts: true
            ...
            var advancedResultsContents = SailPoint.Search.getResultsGrid({
                id: 'advancedAccountGroupSearchResultsGrid',
                type: 'advancedAccountGroup',
                stateful: true,
                stateId: SailPoint.Analyze.gridStateIds.get('AdvancedAccountGroup'),
                url: SailPoint.getRelativeUrl('/analyze/accountGroup/advancedAccountGroupDataSource.json'),
                handleClick: SailPoint.AccountGroup.Search.handleClick,
                pageSize: SailPoint.Analyze.defaultResultsPageSize,
                // for managed attribute, default sorter should be on displayableName not name
                sorters : [{ property: 'displayableName', direction: 'ASC' }],
                optionsPlugin: SailPoint.Search.getOptionsPlugin({
                    searchType: 'AdvancedAccountGroup',
                    cardPanelId: 'accountGroupSearchPanel',
                    searchPanelId: 'advancedAccountGroupSearchContents',
                    applySearchPanelStyles: function() {
                        Ext.getDom('advancedSearchWrapperTbl').style['width'] = '95%';
                        Ext.getCmp('advancedAccountGroupSearchContents').doLayout();
                    },
                    options: [
                        ['saveOrUpdate', '#{msgs.save_search}'],
                        ['saveAsReport', '#{msgs.save_search_as_report}']
        ...
        displayAdvancedSearchResults : function() {
            var searchPanel = Ext.getCmp('accountGroupSearchPanel');
            searchPanel.getLayout().setActiveItem('advancedAccountGroupSearchResultsGridWrapper');
            searchPanel.doLayout();
            if (Ext.isGecko) {
                Ext.getCmp('accountGroupSearchResultsGrid').getView().refresh();
            }
            Ext.MessageBox.hide();
        },
        ...
        displayAdvancedSearch : function() {
            SailPoint.Search.clearForRefineSearch('advancedAccountGroupSearchResultsGrid');

            var searchPanel = Ext.getCmp('accountGroupSearchPanel');
            searchPanel.getLayout().setActiveItem('advancedAccountGroupSearchContents');

            if (!searchPanel.hasDisplayFields) {
                SailPoint.Analyze.SearchDisplayFields.initDisplayFields('advancedAccountGroup');
                searchPanel.hasDisplayFields = true;
            }
            Ext.getCmp('advancedAccountGroupDisplayFieldsPanel').doLayout();
        ...
        finishAdvancedSearchInit : function() {
            advAccountGroupSearchFiltersPage = FiltersPage.instance('div.advAccountGroupSearchspTabledAjaxContent', 'advAccountGroupSearchfilterBeanListTbl', 'advancedAccountGroupSearchForm', 'advAccountGroupSearch');
            advAccountGroupSearchFiltersPage.initPage();
            buildTooltips(Ext.getDom('advancedAccountGroupAttributes'));
            buildTooltips(Ext.getDom('filterCriteria'));
            SailPoint.BaseGrid.initGrid(SailPoint.getRelativeUrl('/analyze/accountGroup/advancedAccountGroupDataSource.json'), 'advancedAccountGroupSearchResultsGrid', 13, true);
            Ext.getDom('advancedSearchWrapperTbl').style['width'] = '95%';
            Ext.getCmp('advancedAccountGroupSearchContents').doLayout();
        },
        ...
        clearSearchFields : function() {
            ...
            // clear the remaining managedAttribute extendedAttribute inputs
            var allInputs = Ext.DomQuery.select('input[id*=Form\:]', Ext.getDom(formName));
            for (i = 0; i < allInputs.length; ++i) {
                var name = allInputs[i].name;
                if (name && name.match(/:\d+:managedAttribute/)) {
                    allInputs[i].value='';
                }
            }
            ...
            var classificationSuggest = Ext.getCmp('accountGroupClassificationSuggest');
            if (classificationSuggest) {
	            classificationSuggest.clearValue();
	            Ext.getDom('accountGroupClassification').value = '';
            }
            ...
            var cloudTypeComboBox = Ext.getCmp('cloudTypeComboBox');
            if (cloudTypeComboBox) {
                cloudTypeComboBox.clearValue();
            }
        ...
        clearAdvancedAccountGroupSearchFields : function() {
            SailPoint.Analyze.resetFieldsToDisplay('advancedAccountGroup');
            advAccountGroupSearchFiltersPage.clearSearchFields();
        },
...
Ext.define('SailPoint.Analyze.AccountGroup', {
    statics : {
        initializeAttributes : function(isGrid) {
            ...
            var classificationSuggest = Ext.getCmp('accountGroupClassificationSuggest');
            var classificationVal = Ext.getDom('accountGroupClassificationVal').innerHTML;
            ...
            if(isGrid) {
                ...
                attributeSuggest = new SailPoint.DistinctRestSuggest({
                    ...
                    suggestUrl: '/rest/analyze/accountGroupSearchPanel/suggest',
                    listeners: {
            ...
            classificationSuggest = new SailPoint.ClassificationSuggest({
                id: 'accountGroupClassificationSuggest',
                renderTo: 'accountGroupClassificationSuggestDiv',
                binding: 'accountGroupClassification',
            ...
            if (cloudTypeComboBox) {
                cloudTypeComboBox.destroy();
            }

            cloudTypeComboBox = new SailPoint.StartWithValueFilterSuggest({
                id: 'cloudTypeComboBox',
                value: SailPoint.Analyze.AccountGroup.getCloudTypeValue(),
                fields: ['label', 'value'],
                suggestUrl: '/analyze/accountGroup/accountGroupCloudTypesDataSource.json',
                storeId:'accountGroupCloudType',
                storeRoot: 'rows',
                displayField: 'label',
                valueField: 'value',
                renderTo: SailPoint.Analyze.AccountGroup.getCloudTypeRenderTo(),
                width: 200,

                getSuggestTpl : function(displayField) {
                    return '<tpl for="."><div class="scopeSearch"><div class="sectionHeader">{label:htmlEncode}</div>' +
                        '<div class="indentedColumn">{value:htmlEncode}</div></div></tpl>';
                },
            });
        ...
        getCloudTypeComp: function() {
            var comp = Ext.getDom('editForm:accountGroupCloudType');
            if (!comp) {
                comp = Ext.getDom('accountGroupSearchForm:accountGroupCloudType');
            }
            return comp;
        },
        getCloudTypeRenderTo: function() {
            var comp = this.getCloudTypeComp();
            return comp ? 'accountGroupCloudTypeDiv' : '';
        },
        getCloudTypeValue: function() {
            var comp = this.getCloudTypeComp();
            return comp ? comp.value : '';
        },
        setCloudTypeValue: function(value) {
            var comp = this.getCloudTypeComp();
            if (!!comp) {
                comp.value = value;
            }
