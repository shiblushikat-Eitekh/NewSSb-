...
SailPoint.Identity.Search.getIdentitySearchPanel = function(config) {
    ...
    var resultsContents = SailPoint.Search.getResultsGrid({
        ...
        optionsPlugin: SailPoint.Search.getOptionsPlugin({
            ...
            identitySearchResultsGridId: 'identitySearchResultsGrid',
    ...
    var advancedResultsContentsConfig = {
        ...
        optionsPlugin: SailPoint.Search.getOptionsPlugin({
            ...
            identitySearchResultsGridId: 'advancedIdentitySearchResultsGrid',
...
SailPoint.Identity.Search.displayAdvancedSearch = function() {
    ...
    // Recreate any special filter inputs (e.g. integer) after panel refresh.
    // For some reason, identity search handles this differently. Other search types use
    // searchResultsGrid.returnToSearchPanel. See earlier in this file for the 'hack' that overrides it.
    advIdentitySearchFiltersPage.createFilterInputs();
};

SailPoint.Identity.Search.initAdvancedSearchPanel = function() {
    advIdentitySearchFiltersPage = FiltersPage.instance('div.advIdentitySearchspTabledAjaxContent', 'advIdentitySearchfilterBeanListTbl', 'advancedIdentitySearchForm', 'advIdentitySearch');
    advIdentitySearchFiltersPage.initPage();
...
SailPoint.Identity.Search.clearAdvancedIdentitySearchFields = function() {
    SailPoint.Analyze.resetFieldsToDisplay('advancedIdentity');
    advIdentitySearchFiltersPage.clearSearchFields();
};
...
SailPoint.Identity.Search.scheduleCertification = function(grid, isAdvanced) {
    ...
    var namesToCertifyName = formName + ':' + ((isAdvanced) ? 'advancedNamesToCertify' : 'namesToCertify');
    ...
    if(grid.selModel.isAllSelected()) {
        ...
        Ext.getDom(namesToCertifyName).value = JSON.stringify(grid.selModel.getSelectedNames());
        Ext.getDom(btnName).click();
    }
};

SailPoint.Identity.Search.roleDiscoveryAI = function(grid, isCommonAccess) {
    var formName = 'identityResultForm';

    var btnName = formName + ':' + 'roleDiscoveryAiBtn';
    var roleDiscoveryAllName = formName + ':' + 'roleDiscoveryAll';
    var roleDiscoveryIdsName = formName + ':' + 'roleDiscoveryIds';
    var commonAccessName = formName + ':' + 'isCommonAccess';

    if (isCommonAccess) {
        Ext.getDom(commonAccessName).value = true;
    } else {
        Ext.getDom(commonAccessName).value = false;
    }

    if(grid.selModel.isAllSelected()) {
        /** If they unselected users, we need to get those and exclude them **/
        if(grid.selModel.getExcludedIds().length > 0) {
            Ext.getDom(roleDiscoveryIdsName).value = arrayToString(grid.selModel.getExcludedIds(), false);
        }

        Ext.getDom(roleDiscoveryAllName).value = 'true';
        Ext.getDom(btnName).click();
    } else if(grid.selModel.getSelectedIds().length < 1) {
        Ext.MessageBox.alert('#{msgs.dialog_no_identities}', '#{msgs.identity_search_err_select_identity_role_mining}');
        return;
    } else {
        Ext.getDom(roleDiscoveryIdsName).value = arrayToString(grid.selModel.getSelectedIds(), false);
        Ext.getDom(btnName).click();
    }
};

SailPoint.Identity.Search.openRoleDiscoveryWindow = function(url) {
