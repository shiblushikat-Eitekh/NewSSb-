...
SailPoint.disableHeader = function(){
    SailPoint.showUnsavedChanges();
    SailPoint.currentChanges.push({action:'disableHeader'});
};
...
SailPoint.reportPreviewInit = function(gridColConf, gridTitle, definitionId, hasSummary, hasChart, hasHeader){
    ...
    if (hasHeader) {
        ...
        Ext.Ajax.request({
            url: SailPoint.getRelativeUrl('/rest/report/'+definitionId+'/header'),
            success: function(response){
                var text = response.responseText;
                var response = Ext.decode(text);
                var tableJson = response.object;

                var headerTables = SailPoint.GridReportLayout.buildHeaderTables(tableJson);

                var container = Ext.getCmp('headerContainer');
                container.add(headerTables);
                headerLoaded = true;
                container.doLayout();
            }
        });
    ...
    if (headerItems.length > 0) {
        var headerPanel = Ext.create('widget.panel', {
            ...
            tools:[{
                type:'close',
                tooltip: "#{msgs.rept_form_field_disable_header}",
                handler: function(event, toolEl, panel){
                    SailPoint.disableHeader();
                    Ext.getCmp("reportHeaderPanel").hide();
                }
            }]
        });
    }
