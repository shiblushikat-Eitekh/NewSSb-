...
SailPoint.Search.getResultsGrid = function(config) {
    ...
    if (withCheckboxes) {
        selectionModel = new SailPoint.grid.CheckboxSelectionModel( {selectMessageBox: Ext.getDom(searchType + 'selectedCount')});
    }
        ...
        store.on('metachange', function(changedStore, newMetaData) {
            ...
            for (i = 0; i < columnConfig.length; ++i) {
                ...
                if (Ext.isDefined(columnConfig[i].header)) {
                    // Encode header in all cases to avoid XSS, we never support HTML in column headers
                    columnConfig[i].header = Ext.String.htmlEncode(columnConfig[i].header);
                }
            }
            
            // flex the remaining width-less columns equally
            Ext.each(columnConfig, function(col) {
              if (!col.width) {
                    col.flex = 1;
                }
            });
...
Ext.define('SailPoint.Search.NumberOfResultsPanel', {
    ...
    constructor: function(config) {
        ...
        config.html = '<div id="' + this.messageId + '" class="spBackground"></div>';
    ...
    setNumberOfResults: function(numberOfResults) {
        ...
        if (Ext.isIE) {
        	...
        	searchInputs = Ext.DomQuery.select('input[class*=searchInputText]', Ext.getDom('roleAttributes'));
...
Ext.define('SailPoint.Search.SearchOptionsPlugin', {
    ...
    constructor: function(config) {
        ...
        this.identitySearchResultsGridId = config.identitySearchResultsGridId;
        this.filtersPageId = config.filtersPageId;
    ...
    onRender: function() {
        ...
        if (this.identitySearchResultsGridId) {
            var resultType, isAdvanced;
            if (this.searchType == 'Identity') {
                resultType = 'Identity';
                isAdvanced = false;
            } else {
                resultType = 'AdvancedIdentity';
                isAdvanced = true;
            }
            var showRoleDiscoveryBtn = false;
            if (userCanAdminRolesAI !== undefined && userCanAdminRolesAI) {
                showRoleDiscoveryBtn = true;
            }

            var showCommonAccessBtn = false;
            if (userCanAdminCommonAccessAI !== undefined && userCanAdminCommonAccessAI) {
                showCommonAccessBtn = true;
            }

            var identitySearchResultsGrid = Ext.getCmp(this.identitySearchResultsGridId);
            this.toolbar.add(new Ext.Button({
                text: '#{msgs.button_schedule_cert}',
                disabled: ((typeof userCanScheduleCerts !== "undefined") ? !userCanScheduleCerts : true),
                handler: function() {
                    Ext.getDom('identityResultForm:searchType').value = resultType;
                    SailPoint.Identity.Search.scheduleCertification(identitySearchResultsGrid, isAdvanced);
                }
            }));
            this.toolbar.add(new Ext.toolbar.Spacer());

            // only create the Role Discovery button if the user has access and identityAIEnabled
            if (showRoleDiscoveryBtn) {
                this.toolbar.add(new Ext.Button({
                    text: '#{msgs.button_ai_access_modeling}',
                    tooltip: '#{msgs.button_ai_access_modeling_tooltip}',
                    disabled: (!showRoleDiscoveryBtn),
                    handler: function () {
                        Ext.getDom('identityResultForm:searchType').value = resultType;
                        SailPoint.Identity.Search.roleDiscoveryAI(identitySearchResultsGrid, false);
                    }
                }));
                this.toolbar.add(new Ext.toolbar.Spacer());
            }

            // only create the Common Access button if the user has access and identityAIEnabled
            if (showCommonAccessBtn) {
                this.toolbar.add(new Ext.Button({
                    text: '#{msgs.button_ai_common_access}',
                    tooltip: '#{msgs.button_ai_common_access_tooltip}',
                    disabled: (!showCommonAccessBtn),
                    handler: function () {
                        Ext.getDom('identityResultForm:searchType').value = resultType;
                        SailPoint.Identity.Search.roleDiscoveryAI(identitySearchResultsGrid, true);
                    }
                }));
                this.toolbar.add(new Ext.toolbar.Spacer());
            }
        ...
        if (searchType != 'Syslog' && searchType != 'AdvancedSyslog') {
            this.toolbar.add(new Ext.button.Button({
                iconCls: 'pdfIcon',
                handler: function() {
                    Ext.getDom('searchSaveForm:searchType').value = searchType;
                    SailPoint.Analyze.exportReport('pdf');
                },
                tooltip: '#{msgs.export_as_pdf}'
            }));
        }
        ...
        this.toolbar.add(new Ext.button.Button({
            ...
            tooltip: '#{msgs.export_as_csv}'
        }));

        // HP ArcSight Integration - IdentityIQ CEF data feed
        if (searchType == 'Identity' || searchType == 'Audit' || searchType == 'Syslog'
            || searchType == 'Link' || searchType == 'AdvancedSyslog') {
            this.toolbar.add(new Ext.button.Button({
                iconCls: 'cefIcon',
                handler: function() {
                    Ext.getDom('searchSaveForm:searchType').value = searchType;
...
//  	if (Ext.get('stateForm')) {
//  	Ext.getDom('stateForm:currentCardPanel').value = this.initialConfig.searchPanelId;
...
//  	}
    	...
    	let filtersPageId;
    	if (this.searchType && this.searchType.startsWith('Advanced')) {
    	    filtersPageId = this.searchType.replace('Advanced', 'adv') + 'SearchFiltersPage';
        }

        if (filtersPageId && window[filtersPageId]) {
    	    window[filtersPageId].createFilterInputs();
        }
    },
    ...
    saveSearch: function (saveOption, forced) {
        ...
        Ext.getDom(formName + ':searchType').value = this.searchType;
        if ( selValue == 'saveOrUpdate' ) {
            SailPoint.Analyze.saveQuery(Ext.getDom(formName + ':rememberQuery'), '#{msgs.save_search}', 'search_lcase');
        } else if ( selValue == 'saveAsReport' ) {
            SailPoint.Analyze.saveQuery(Ext.getDom(formName + ':saveQueryAsReport'), '#{msgs.save_search_as_report}', 'report_lcase');
        } else if ( selValue == 'saveAsPopulation' ) {
            SailPoint.Analyze.saveQuery(Ext.getDom(formName + ':saveQueryAsIpop'), '#{msgs.save_identities_as_population}', 'population_lcase');
            ...
            SailPoint.Analyze.saveQuery(Ext.getDom(formName + ':rememberQueryAsIdentity'), '#{msgs.save_search_as_identity_search}', 'search_lcase');
        }
        saveOption.setValue('');
    }
});
...
SailPoint.Search.handleClick = function(grid, rowIndex, columnIndex, e) {
    ...
    if(col) {
        Ext.getDom('resultsForm:currentObjectId').value = record.getId();
        Ext.getDom('resultsForm:editButton').click();
    } else {
        e.stopEvent();
    }
};
...
SailPoint.Search.clearForRefineSearch = function(checkboxGridId) {
	...
	if (msgsUlList && msgsUlList.length > 0) {
		var msgsUl = Ext.get(msgsUlList).first().dom;
		msgsUl.innerHTML="";
