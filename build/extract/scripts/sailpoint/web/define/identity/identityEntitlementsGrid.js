...
Ext.define('SailPoint.Define.Grid.IdentityEntitlements', {
    statics : {
        ...
        renderAttribute : function(value, p, r) {
          ...
          // IIQFW-2: Encode attribute name to avoid HTML injection
          ...
          if(r.raw.iiqElevatedAccess && r.raw.permission) {
            value += ' <img class="iiq-elevated-access-icon iiq-elevated-access-icon-image" ' +
        ...
        renderValue : function(value, p, r) {
            ...
            if (name && name.length > 0) {
                ...
                if (appName != null) {
                    ...
                    // earlier in this function, and may now be wrapped in other HTML that we want to render.
                    if (isGroup) {
                        ...
                        rendered = '<a onclick="javascript: SailPoint.Define.Grid.IdentityEntitlements.viewAccountGroup(\''
                                + Ext.String.htmlEncode(appName) + '\', \'' + Ext.String.htmlEncode(name) + '\', \'' + value + '\', event);">' +
                            SailPoint.grid.Util.renderIiqElevatedAccessUnsafe(nameWithTooltip, null,  r) + '</a>';
                    } else {
                        rendered = '<div class="font10">' + SailPoint.grid.Util.renderIiqElevatedAccessUnsafe(nameWithTooltip, null,  r) + "</div>";
                    }
                }
