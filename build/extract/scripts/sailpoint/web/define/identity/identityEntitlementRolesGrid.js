...
Ext.define('SailPoint.Define.Grid.IdentityEntitlementRoles', {
    statics : {
        ...
        renderValue : function(value, p, r) {
            ...
            var hasClassifications = !Ext.isEmpty(r.get('classificationNames'));
            ...
            if ( name && name.length > 0 ) {
                ...
                if ( name === "assignedRoles" || name === "detectedRoles" ) {
                    ...
                    if ( roleId != null ) {
                          ...
                          roleLink = Ext.String.format('<a href="javascript:SailPoint.RoleDetailPanel.window(null, \'{0}\', \'{1}\', true, null, \'{2}\', undefined, undefined, \'identity\', \'{1}\', {4})" title="#{msgs.info_role_composition}">{3}</a>', roleId, identityId, name, rendered, hasClassifications);
                    		} else {
                          roleLink = Ext.String.format('<a href="javascript:SailPoint.RoleDetailPanel.window(\'{0}\', \'{1}\', \'{2}\', true, null, \'{3}\', undefined, undefined, \'identity\', \'{2}\', {5})" title="#{msgs.info_role_composition}">{4}</a>', assignmentId, roleId, identityId, name, rendered, hasClassifications);
                    		}
                        var roleType = r.get('roleType');
                        var attributes = r.get('attributes')
                        var typeIcon = attributes ? attributes.roleTypeIcon : null;
                        // Using the "unsafe" version of renderIiqElevatedAccess` since the value was escaped
                        // earlier in this function, and is now wrapped in other HTML that we want to render.
                        if ( roleType == null ) {
                            rendered = '<div class="font10">' + SailPoint.grid.Util.renderIiqElevatedAccessUnsafe(roleLink, null,  r) + "</div>";
                        } else {
                            rendered = '<div style="padding-left:18px" class="font10 ' + typeIcon + '">' + SailPoint.grid.Util.renderIiqElevatedAccessUnsafe(roleLink, null,  r) + "</div>";
                        }
                    }
                } 
                ...
                if(Ext.get('editForm:roleId') && roleId == Ext.getDom('editForm:roleId').value) {
                    var sunsetTime;
                    var sunriseTime;
                    var sunsetD;
                    var sunriseD;
                    sunriseTime = Ext.getDom('editForm:sunriseDate').value;
                    sunsetTime = Ext.getDom('editForm:sunsetDate').value;
                    if(sunriseTime) {
                ...
                if ( sunrise || sunset ) {
                    ...
                    var activationEdit;
                    var baseDiv =
                        '<div class="activationNotice allowWrap" style="margin: 10px 0pt">'
                        + '{0}'
                        + '{1}'
                        + '{2}'
                       +'</div>';
                    if(Ext.get('editForm:sunriseDate')) {
                        activationEdit = '<span class="activationNoticeItem activationEdit">'
                    ...
                    if ( sunrise ) {
                        sunriseDiv = '<span class="activationNoticeItem">'
                    ...
                    if ( sunset ) { 
                        sunsetDiv = '<span class="activationNoticeItem">'
        ...
        /**
         * Value is a string left padded with zeroes between 0000000001 and 9999999999.
         * Drop all the leading zeroes
         * @param value The value to transform
         * @returns {string} The transformed value
         */
        renderRequestId: function(value) {
            ...
            initExpandoPanel : function(grid) {
                ...
                var sourceStore = SailPoint.Store.createStore({
                    ...
                    data : [ { key : 'LCM', value : '#{msgs.source_lcm}' },
                             { key : 'RapidSetup', value : '#{msgs.source_rapidsetup}' },
