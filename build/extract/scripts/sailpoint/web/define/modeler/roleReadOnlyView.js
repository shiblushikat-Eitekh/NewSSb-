        ...
        '<div class="spTabledContent"  style="margin: 6px">',
          '<table class="paddedTbl width100">',
            '<tbody>',
              ...
              '<tr><td class="titleColumn">{classificationI18n}</td><td colspan="2"><span>{roleClassification}</span></td></tr>',
              '<tr><td class="titleColumn">{iiqElevatedAccessI18n}</td><td colspan="2"><span>{iiqElevatedAccess}</span></td></tr>',
              '<tpl if="roleActivityMonitoringIsEnabled">',
                '<tr><td class="titleColumn" colspan="3">{roleActivityMonitoringEnabled}</td></tr>',
              '</tpl>',
              '<tpl if="roleMergeTemplates">',
                '<tr><td class="titleColumn" colspan="3">{roleMergeTemplates}</td></tr>',
              '</tpl>',
...
Ext.ns('SailPoint', 'SailPoint.Role', 'SailPoint.Role.RoleView', 'SailPoint.grid.Util');
    ...
    if (results.roleName) {
        ...
        if (hasEntitlements) {
            ...
            if (results.hasSimpleIncludedEntitlements) {
                ...
                //IIQTC-83 :- When clicking in a new node we need to refresh the current
                //page for inherited entitlements
                simpleIncludedEntitlementsGridDataStore.currentPage = 1;
...
SailPoint.Role.RoleView.displayRoleReference = function(displayOptions) {
    ...
    if (isShown) {
        ...
        gridPanel.show();
        sectionPanel.show();
        //IIQTC-83 :- When clicking in a new node we need to refresh the current page
        //for inherited roles
        store.currentPage = 1;
...
SailPoint.Role.RoleView.getRoleViewContentPanel = function(config) {
    ...
    var directEntitlementsSectionSimpleConfig = SailPoint.Role.RoleView.createSimpleEntitlementPanel({
        ...
        columns: [
            ...
            { dataIndex: 'displayValue', header: '#{msgs.role_simple_entitlement_value_header}', flex: 3, sortable: true, renderer: SailPoint.grid.Util.renderIiqElevatedAccess },
