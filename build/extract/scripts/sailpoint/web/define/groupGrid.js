...
SailPoint.Define.Grid.Group.createGroupsGrid = function(fields, columns, gridStateStr, pageSize, stateId, gridWidth) {
    ...
    return {
        ...
        tbar : [ {
            ...
            scale : 'medium',
            cls : 'primaryBtn',
            disabled: Ext.getDom('editForm:createGroupButton').disabled,
            handler : function() {
                Ext.getDom("editForm:createGroupButton").click();
            }
        } ],
        listeners : {
  ...
  workgroupsGrid = Ext.create('SailPoint.grid.PagingGrid', {
    ...
    tbar: [
      ...
      ' ',
      {
        ...
        scale : 'medium',
        cls : 'primaryBtn',
        disabled: Ext.getDom('editForm:createWorkgroupButton').disabled,
        handler: function() {
          Ext.getDom("editForm:createWorkgroupButton").click();
        }
      }],
      listeners : {
          activate : SailPoint.Define.Grid.Group.refreshPanel,
...
SailPoint.Define.Grid.Group.deletePrompt = function(item) {
    ...
    var name = Ext.String.htmlEncode(item.name ? item.name : item);
...
SailPoint.Define.Grid.Group.deleteObject = function(button, text) {
    if (button == 'yes') {
      switch (root) {
        case 'groups':
            Ext.getDom('editForm:currentGroupObjectId').value = id;
            Ext.getDom('editForm:deleteGroupButton').click();
                            
            break;
        case 'populations':
            Ext.getDom('editForm:currentPopsObjectId').value = id;
            Ext.getDom('editForm:deletePopButton').click();
        ...
        // case 'accountGroups':
        case 'objects':
            Ext.getDom('editForm:currentAccountGroupObjectId').value = id;
            Ext.getDom('editForm:deleteAccountGroupButton').click();
                            
            break;
        case 'workgroups':
            Ext.getDom('editForm:currentWorkgroupObjectId').value = id;
            Ext.getDom('editForm:deleteWorkgroupButton').click();
            break;
        default:
...
SailPoint.Define.Grid.Group.generateDescriptions = function(record) {


    const GENAI_ENTITLEMENT_DESCRIPTION = "GenAIEntitlementDescription";

    const selectedIds =
        record.length === 0 ? [id]
        : record.map(function(value) {return value.data.id;});

    Ext.Ajax.request({
        url: SailPoint.getRelativeUrl('/rest/asyncRequest'),
        method: 'POST',
        jsonData: {
            data: JSON.stringify({ entitlementIds: selectedIds, asyncRequestType: GENAI_ENTITLEMENT_DESCRIPTION})
        },
        success: function(response) {
            location.replace(SailPoint.getRelativeUrl("/define/groups/entitlementDescription.jsf"));
        },
        failure: function(response) {
            Ext.Msg.alert('Error', '#{msgs.label_async_request_failure_message}');
        }
    });
};
...
SailPoint.Define.Grid.Group.editObject = function(item) {
  ...
  switch (root) {
    case 'subgroups':
        ...
        Ext.getDom('groupForm:currentSubGroupObjectId').value = id;
        Ext.getDom('groupForm:editSubGroupButton').click();
                        
        break;
    case 'groups':
        grid.panel.gridState.encodeGridState('editForm:G');
        Ext.getDom('editForm:currentGroupObjectId').value = id;
        Ext.getDom('editForm:editGroupButton').click();
                        
        break;
    case 'populations':
        grid.panel.gridState.encodeGridState('editForm:P');
        Ext.getDom('editForm:currentPopsObjectId').value = id;
        Ext.getDom('editForm:editPopButton').click();
    ...
    case 'objects':
        ...
        Ext.getDom('editForm:currentAccountGroupObjectId').value = id;
        Ext.getDom('editForm:editAccountGroupButton').click();
                        
        break;
     case 'workgroups':
        grid.panel.gridState.encodeGridState('editForm:WG');
        Ext.getDom('editForm:currentWorkgroupObjectId').value = id;
        Ext.getDom('editForm:editWorkgroupButton').click();
...
SailPoint.Define.Grid.Group.refreshPanel = function(component) {
    ...
    var createBtn = Ext.getDom('editForm:createGroupButton');
    ...
    var createWorkgroupBtn = Ext.getDom('editForm:createWorkgroupButton');
...
SailPoint.Define.Grid.Group.isEditAndDeleteDisabled = function(gridView, isGenAiCapabilityExist) {
    var selModel = gridView.getSelectionModel().getSelection();
    if (isGenAiCapabilityExist && selModel.length > 1) {
      return isGenAiCapabilityExist;
    }
    return false;
};
...
SailPoint.Define.Grid.Group.showContextMenu = function(gridView, record, HTMLitem, index, event, eOpts) {
  ...
  if(root == "workgroups") {
      deleteBtn = Ext.getDom('editForm:deleteWorkgroupButton');
  } else if (root == "groups") {
      deleteBtn = Ext.getDom('editForm:deleteGroupButton');
  } else if (root == "populations") {
      deleteBtn = Ext.getDom('editForm:deletePopButton');
  } else if (root == "objects") {
      deleteBtn = Ext.getDom('editForm:deleteAccountGroupButton');
  }
  ...
  var gridViewData = gridView.ownerCt;
  var isGenAiCapability = gridViewData.genAiCapability;
  var entitlementDescriptionStatus = gridViewData.entitlementStatus;

  if (isGenAiCapability) {
      var selModel = gridView.getSelectionModel();
      var records = selModel.getSelection();
      contextMenu.add(
          new Ext.menu.Item({
              text: '#{msgs.menu_generate_descriptions}',
              handler: function () {
                  SailPoint.Define.Grid.Group.generateDescriptions(records);
              },
              iconCls: 'generateDescription',
              theGrid: gridView,
              disabled: entitlementDescriptionStatus.includes(record.raw.genai_description_request_status)
          })
      );
  }
  contextMenu.add(
      new Ext.menu.Item({text: '#{msgs.menu_edit}', 
                         ...
                         iconCls: 'editBtn',
                         theGrid: gridView,
                         disabled: SailPoint.Define.Grid.Group.isEditAndDeleteDisabled(gridView,isGenAiCapability)})
  );
  ...
  if (root != 'subgroups') {
    contextMenu.add(
      new Ext.menu.Item({text: '#{msgs.menu_delete}', 
                         ...
                         iconCls: 'deleteBtn',
                         disabled: deleteDisabled || SailPoint.Define.Grid.Group.isEditAndDeleteDisabled(gridView,isGenAiCapability),
                         name: name})
    );          
  }
