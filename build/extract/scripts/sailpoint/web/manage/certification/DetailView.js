...
SailPoint.certification.detailNext = function(){
    var id = Ext.getDom('nextEntity').value;
...
SailPoint.certification.detailPrev = function(){
    var id = Ext.getDom('prevEntity').value;
    SailPoint.CurrentEntityIndex--;
...
SailPoint.certification.initDetailView = function(doSecondPass){
    ...
    var detailGridIds = [];
    Ext.each(detailGrids, function(grid){
        detailGridIds.push(grid.id);
    });

    if (itemId){
        for(var i=0; i<detailGrids.length; i++) {
            if (!detailGrids[i].baseParams) {
                detailGrids[i].baseParams = {}
            }
            detailGrids[i].baseParams["itemId"] = itemId;
        }
    }
    ...
    Ext.each(SailPoint.certification.initDetailTabs(SailPoint.CurrentEntityDetails.type, SailPoint.WorkItemId, itemId),
        function(tab){
            tabs.push(tab);
        }
    );
    ...
    //Moving some of this disable call to detailViewInclude since it's too much of a hassle to
    //check for workgoup membership at this level.
    if(SailPoint.isEntityDelegated && !SailPoint.CurrentEntityDetails.workItemId ) {
...
SailPoint.certification.goBack = function() {
    if(Ext.getDom('editForm:goBack'))
            Ext.getDom('editForm:goBack').click()
};
        ...
        success: function(response){
            ...
            if (!respObj.success){
                ...
                if(!delegationOnly) {
                  ...
                  Ext.getDom('nextEntity').value = data.nextEntity;
                  Ext.getDom('prevEntity').value = data.prevEntity;
                }
...
SailPoint.certification.initGrid = function(type, title, entityId){
    ...
    var baseParams = {
        colKey: SailPoint.colKeys[type]
    };
    if (workItemId) {
        baseParams.workItemId = workItemId;
    }
    ...
    if (gridConfig.gridMetaData) {
        Ext.each(gridConfig.gridMetaData.columns, function(column){
            column.onTitleMouseOver = Ext.emptyFn;
            column.onTitleMouseOut = Ext.emptyFn;

            // Check each column to make sure those beginning with IIQ_ are not sortable since these are
            // generated columns by IIQ.  See CALCULATED_COLUMN_PREFIX static in in BaseListResource.java
            if (column.sortable && column.dataIndex.indexOf('IIQ_') == 0) {
...
SailPoint.certification.initDetailTabs = function(entityType, workItemId, itemId){
    var tabs = [];
        ...
        tabs.push(
            {
                ...
                url:SailPoint.getRelativeUrl('/manage/certification/tabContent/groupDetails.jsf?workItemId='+workItemId+'&itemId='+itemId+'&entityId=')
            }
        );
