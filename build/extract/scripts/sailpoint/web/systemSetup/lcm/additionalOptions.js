...
SailPoint.systemSetup.lcm.initAdditionalAccountAppMultiSuggest = function(addtAcctApps) {
    SailPoint.systemSetup.lcm.initAccountAppMultiSuggest(SailPoint.systemSetup.lcm.ADDITIONAL_ACCOUNT_APPS_COMPONENT, Ext.getDom('editForm:additionalAccountAppsCheckbox'), addtAcctApps);
};

SailPoint.systemSetup.lcm.initAccountOnlyAppMultiSuggest = function(acctOnlyApps) {
    SailPoint.systemSetup.lcm.initAccountAppMultiSuggest(SailPoint.systemSetup.lcm.ACCOUNT_ONLY_APPS_COMPONENT, Ext.getDom('editForm:accountOnlyAppsCheckbox'), acctOnlyApps);
};

SailPoint.systemSetup.lcm.initDisableAutoRefreshAcctStatusAppMultiSuggest = function(disableAutoRefAcctApps) {
    SailPoint.systemSetup.lcm.initAccountAppMultiSuggest(SailPoint.systemSetup.lcm.DISABLE_AUTO_REFRESH_ACCOUNT_STATUS_APPS_COMPONENT, Ext.getDom('editForm:disableAutoRefreshAccountStatusCheckbox'), disableAutoRefAcctApps);
};
...
SailPoint.systemSetup.lcm.toggleEnableFullText = function(el) {
    if (Ext.getDom('editForm:lcmEnableFulltext').checked) {
        Ext.getDom('editForm:searchType').disabled = true;
    }
    var i, elem,
        interval = Ext.getDom('editForm:lcmFullTextInterval'),
        mode = Ext.getDom('editForm:lcmFullTextExecutionMode'),
        path = Ext.getDom('editForm:lcmFullTextIndexPath'),
        elems = [path, mode];
...
SailPoint.systemSetup.lcm.enableFullTextEvent = function(el) {
    Ext.getDom('editForm:searchType').disabled = Ext.getDom('editForm:lcmEnableFulltext').checked;
    var type = Ext.getDom('editForm:searchType');
    if (!el.checked) {
        type.value = 'contains';
    }else{
    ...
    SailPoint.systemSetup.lcm.toggleEnableFullText(el);
};

SailPoint.systemSetup.lcm.toggleExecutionMode = function(el) {
    var interval = Ext.getDom('editForm:lcmFullTextInterval');
    if (interval) {
...
Ext.onReady(function() {
    SailPoint.systemSetup.lcm.toggleEnableFullText(Ext.getDom('editForm:lcmEnableFulltext'));
    SailPoint.systemSetup.lcm.toggleExecutionMode(Ext.getDom('editForm:lcmFullTextExecutionMode'));
});
