...
function showTimeoutMsg() {
  if(!SailPoint.getTimeoutLock()) {
      ...
      Ext.Ajax.request({
          url: CONTEXT_PATH + '/ui/rest/checkSession',
          failure: function() {
    ...
    Ext.Msg.show(
      {title: "#{msgs.session_expiration_title}",
       ...
       fn: function(win) {
             ...
             let redirectUrlHash = document.getElementById('sessionTimeoutForm:preRedirectUrlHash');
             //last chance to try and preserve the hash.
             if (null !== hash && hash.length > 0 && redirectUrlHash) {
                  redirectUrlHash.value = window.location.hash;
             }
