...
Ext.define('SailPoint.GroupSuggest', {
    ...
    getSuggestTpl : function() {
        var suggestTpl = '<tpl for="."><div class="groupSearch"><div class="sectionHeader">{name:htmlEncode}</div>'
                + '<div class="indentedColumn">{factory:htmlEncode}</div></div></tpl>';
        if (this.displayField !== 'name') {
            suggestTpl = '<tpl for="."><div class="groupSearch">' +
            '<div class="sectionHeader">{displayableValue:htmlEncode}</div></div></tpl>';
        }
        return suggestTpl;
    },
    ...
    initComponent : function() {
        ...
        Ext.applyIf(this.extraParams, params);
...
Ext.define('SailPoint.ManagedAttributeIdentitySuggest', {
    ...
    initComponent: function() {
        ...
        this.on('beforerender', function() {
            ...
            // If suggestUrl is defined, use it! otherwise the base suggest wont authorize this column.
            var baseUrl = this.suggestUrl || 'rest/suggest',
                url = SailPoint.getRelativeUrl(baseUrl + '/column/' + this.className + "/" + this.column);
...
Ext.define('SailPoint.DistinctRestSuggest', {
    ...
    initComponent : function() {
        ...
        var params = this.baseParams || {},
            urlBase = this.suggestUrl || '/rest/suggest';
        Ext.applyIf(params, this.extraParams);
        ...
        this.store = SailPoint.Store.createStore({
            fields : [ 'id', 'displayName' ],
            url : SailPoint.getRelativeUrl(urlBase + '/column/' + this.className + "/" + this.column),
            extraParams : params,
            root : 'objects',
            totalProperty : 'count',
            storeId : this.getStoreId(),
 ...
 * SailPoint object
 */
Ext.define('SailPoint.ObjectRestSuggest', {
    extend : 'SailPoint.BaseSuggest',
    alias : 'widget.objectRestSuggest',
    ...
    initComponent : function() {
        ...
        this.store = SailPoint.Store.createStore({
            fields : fields,
            url : SailPoint.getRelativeUrl(urlBase + '/object/' + this.className),
            extraParams : params,
...
Ext.define('SailPoint.ClassificationSuggest', {
    extend : 'SailPoint.ObjectRestSuggest',
    
    suggestUrl: '/rest/suggest',

    className: 'Classification',

    fields:  [ 'id', 'displayName', 'origin' ],
    
    getSuggestTpl : function(displayField) {
        return '<tpl for="."><div class="scopeSearch"><div class="sectionHeader">{displayName:htmlEncode}</div>'
            + '<div class="indentedColumn">{origin:htmlEncode}</div></div></tpl>';
    }
});

Ext.define('SailPoint.AppSuggest', {
    extend : 'SailPoint.ObjectRestSuggest',
    ...
    getSuggestTpl : function(displayField) {
        return '<tpl for="."><div class="scopeSearch"><div class="sectionHeader">{name:htmlEncode}</div>'
            + '</div></tpl>';
    }
});

/**
 * @class SailPoint.StartWithValueFilterSuggest
 * @extends SailPoint.Suggest
 *
 */
Ext.define('SailPoint.StartWithValueFilterSuggest', {
    extend : 'SailPoint.Suggest',
    alias : 'widget.startWithValueFilterSuggest',
    ...
    initComponent : function() {
        ...
        let storeSortOrder = this.storeSortOrder;
        if (!storeSortOrder) {
            storeSortOrder =  'ASC';
        }
        ...
        this.store = SailPoint.Store.createStore ({
            fields: fields,
            url: SailPoint.getRelativeUrl(this.suggestUrl),
            root: this.storeRoot,
            storeId : this.getStoreId()
        });

        this.store.sort(valueField, storeSortOrder);
        const startWithValueFilterId = this.getId() + '-' + 'startWithValueFilter';

        this.addListener('change', function(combo, newVal, oldVal, eOpts) {
            if (this.getStore()) {
                let storeFilters = this.getStore().filters;
                let startWithValueFilter;
                if (storeFilters && storeFilters.items) {
                    startWithValueFilter = storeFilters.items.find(
                        item => item.id === startWithValueFilterId);
                }
                if (startWithValueFilter) {
                    storeFilters.remove(startWithValueFilter);
                }

                startWithValueFilter = SailPoint.Suggest
                    .createStartWithValueFilter(startWithValueFilterId, valueField, newVal);
                storeFilters.add(startWithValueFilter);
            }
            SailPoint.Analyze.AccountGroup.setCloudTypeValue(combo.getValue());
        });
    },
});
...
SailPoint.SuggestFactory = {
    ...
    createSuggest : function(suggestType, renderTo, exclusionIds, emptyText, config) {
        ...
        case 'group':
            ...
            Ext.applyIf(suggestConfig, otherParams);
        ...
        case 'managedAttribute':
            ...
            break;
        case 'classification':
            suggest = new SailPoint.ClassificationSuggest(suggestConfig);
            break;
        default:
...
/**
 * Utility function used to create filter for Suggest Value's list
 * to show component's with startValue started from specific start substring only.
 *
 * @param {String} filterId The filter's id in the object store
 * @param {String} valueField The field in the data used for filter
 * @param {String} startValue The substring is start part for values should be shown in list
 *
 * @return {Ext.util.Filter} created filter
 */
SailPoint.Suggest.createStartWithValueFilter =
    function(filterId, valueField, startValue) {
        return Ext.create('Ext.util.Filter', {
            id: filterId,
            filterFn: function (record, val) {
                return (SailPoint.Utils.isNullOrEmpty(startValue) ||
                    record.data[valueField].toUpperCase().startsWith(startValue.toUpperCase()));
            }
    });
};
