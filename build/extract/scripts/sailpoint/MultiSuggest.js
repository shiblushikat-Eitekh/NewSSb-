...
Ext.define('SailPoint.BaseMultiSuggest', {
    ...
    */
    ...
    /**
     * @cfg {boolean} This determines when an item is added to the suggest whether we
     * encode the text before adding it to the store. Setting to false by default
     * to avoid potential regression issues with all Suggest components.
     */
    encodeOnAddNew : false,
    ...
    initComponent: function() {
        ...
        if (this.comboWidth || this.width) {
            this.suggest.width = this.comboWidth ? this.comboWidth : this.width;
        } else {
            // default to auto width
            this.suggest.anchor = '100%';
        }
...
//            }
    ...
    disable: function() {
        ...
        var inputField = Ext.getDom(this.inputFieldName);
        if(inputField) {
    ...
    addItem: function(suggest, record, index) {
        ...
        // Do not encode the displayField here but instead let the renderer encode the field
        record[0].set('displayField', rField);
    ...
    addNewValue: function(evt) {
        ...
        // IIQETN-4867 encode html before adding to store
        if (this.encodeOnAddNew) {
    ...
    getInputField: function() {
        return Ext.getDom(this.inputFieldName);
    },
...
Ext.define('SailPoint.MultiSuggest', {
    ...
    initComponent:function() {
        ...
        if (this.width) {
            gridConfig.columns.width = this.width;
        }
    ...
    statics: {
        renderRemove: function(value, p, rec, rowIndex, colIndex, store) {
            ...
            // Encode the value to be safely displayed in html for this renderer
            if (value) {
...
Ext.define('SailPoint.CompactMultiSuggest', {
    ...
    initComponent: function() {
        ...
        var replaceArgs = (this.encodeOnAddNew) ? '/&#39;/g, "\\\\&#39;"' : '"\\\'", "\\\\\'"';
        ...
        this.listPanelTpl = new Ext.XTemplate(
            ...
            '  <img onclick="Ext.getCmp(\'' + this.id + '\').removeClicked(\'{[ values.id.replace(' + replaceArgs + ') ]}\');"',
    ...
    removeClicked: function(id) {
        ...
        // IIQETN-4867 escape id before trying to find id
        if (this.encodeOnAddNew) {
