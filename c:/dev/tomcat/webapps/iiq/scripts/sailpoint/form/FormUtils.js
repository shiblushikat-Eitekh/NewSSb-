...
Ext.define('SailPoint.form.Util', {
    statics : {
        ...
        updateFormPanel : function (config) {
            ...
            if(!Ext.getDom(containerName+':form-config-'+formId)) {
            ...
            var htmlDecodeString = Ext.String.htmlDecode(Ext.getDom(containerName + ':form-config-' + formId).innerHTML);
            var formConf;
            if (htmlDecodeString.trim() === '{}' || htmlDecodeString.trim() === '') {
                formConf = null;
            } else {
                formConf = Ext.decode(htmlDecodeString);
            }
