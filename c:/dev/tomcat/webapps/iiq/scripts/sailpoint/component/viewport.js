...
//    var instructions = config.instructions;
    ...
    // Title for pages using combination of Ext and Angular
    var pageTitle = {
        id : 'title',
        cls: 'pageTitle',
        xtype : 'container',
        ...
        listeners : {
            beforerender : function() {
                var me = this;
                if (Ext.get('pageTitle')) {
                    me.contentEl = 'pageTitle';
                }
            }
        }
    }
    ...
    var viewportListeners = {
        afterlayout: {
            fn: function() {
                var headerHeight = Ext.get('spHeaderPanel').getHeight(),
                    menuHeight = Ext.get('menuMainDiv').getHeight(),
                    pageTitleHeight,
                    bodyHeight;

                Ext.get('title').setStyle({
                    top: headerHeight + 'px'
                });
                pageTitleHeight = Ext.get('title').getHeight();

                Ext.get('spBodyPanel').setStyle({
                    top: headerHeight + pageTitleHeight + 'px'
                });    
                bodyHeight = Ext.get('spBodyPanel').getHeight();

                Ext.get('spFooterPanel').setStyle({
                    top: (headerHeight + pageTitleHeight + bodyHeight) + 'px'
                });
                
                // Set the top and height of the quicklink panel to stretch the whole viewport under the menu bar
                jQuery('#quicklinkPanel').css({
                    height: this.getHeight() - menuHeight
                });
    ...
    var spViewport = Ext.create('SailPoint.component.Viewport', {
        ...
        items : [ {
            ...
            baseCls : 'borderCell',
            listeners : {
                // Angular pages does not have spFooterPanelDiv , instead use bodyDivFooter
                beforerender : function() {
                    var me = this;
                    if (!Ext.get('spHeaderPanelDiv')) {
                        me.contentEl = 'formHeader';
                    }
                }
            }
        },
        ...
        spBodyPanel, // content!
        {
            ...
            listeners : {
                ...
                beforerender : function() {
                    var me = this;
                    if (!Ext.get('spFooterPanelDiv')) {
                        me.contentEl = 'bodyDivFooter';
                    }
                }
            }
        } ]
