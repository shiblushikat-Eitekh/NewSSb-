    ...
    Ext.Ajax.request({
        ...
        success : function(response) {
            ...
            try {
                ...
                validAccess = false;
                // got non-JSON data back - likely access denied
                alert('#{msgs.rule_editor_insufficient_rights_for_rule_edit}');
...
SailPoint.Rule.Editor.saveRule = function(wndw, btn) {
    Ext.getDom('ruleEditorForm:ruleName').value = Ext.getCmp('ruleName').getValue();
    Ext.getDom('ruleEditorForm:ruleId').value = Ext.getCmp('ruleId').getValue();
    Ext.getDom('ruleEditorForm:ruleSource').value = Ext.getCmp('source').getValue();
    Ext.getDom('ruleEditorForm:ruleDescription').value = Ext.getCmp('description').getValue();
...
SailPoint.Rule.Editor.postValidateSaveRule = function(rName) {
    SailPoint.Rule.Editor.saveRule(SailPoint.Rule.Editor.EditWindow, Ext.getDom('ruleEditorForm:saveRule'));
};
...
SailPoint.Rule.Editor.postValidateSaveRuleAs = function(rName) {
    ...
    SailPoint.Rule.Editor.saveRule(SailPoint.Rule.Editor.SaveAsWindow, Ext.getDom('ruleEditorForm:saveRuleAs'));
};
...
SailPoint.Rule.Editor.refresh = function() {
    if (SailPoint.Rule.Editor.refreshBtn){
        if (SailPoint.Rule.Editor.refreshBtn.click) {
            SailPoint.Rule.Editor.refreshBtn.click();
        }
        else if (SailPoint.Rule.Editor.refreshBtn.dom && SailPoint.Rule.Editor.refreshBtn.dom.click) {
            SailPoint.Rule.Editor.refreshBtn.dom.click();
        }
    }
    ...
    // editor
    var ruleNameField = Ext.getDom('ruleEditorForm:ruleName');
    if (SailPoint.Rule.Editor.sourceCombo && ruleNameField) {
        var ruleName = ruleNameField.value;
        SailPoint.Rule.Editor.sourceCombo.setValue(ruleName);
