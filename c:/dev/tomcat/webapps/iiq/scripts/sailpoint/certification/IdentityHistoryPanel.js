...
SailPoint.IdentityHistoryPanel.getStoreInstance = function(itemId, identity, workItemId){
    var wiParam = '';
    if (workItemId) {
        wiParam = '&workItemId=' + workItemId;
    }
    return SailPoint.Store.createRestStore({
        url: SailPoint.getRelativeUrl('/rest/identities/' + SailPoint.Utils.encodeRestUriComponent(identity)+ '/history/byItem?itemId=' + itemId + wiParam),
        remoteSort:true,
        method: 'GET',
        root: 'objects',
        totalProperty: 'count',
...
SailPoint.IdentityHistoryPanel.getPanel = function(itemId, identity, targetEl, workItemId) {

    var grid = Ext.create('SailPoint.grid.PagingGrid', {
        store: SailPoint.IdentityHistoryPanel.getStoreInstance(itemId, identity, workItemId),
 ...
 */
SailPoint.IdentityHistoryPanel.showHistoryCommentDialog = function(itemId, workItemId) {
    SailPoint.Manage.Certification.addEntitlementComment(itemId, function() {
        // update the history grid if it's open
        Page.fireEvent("addEntitlementComment", itemId); },
        workItemId
    );
};
