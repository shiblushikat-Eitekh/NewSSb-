    ...
    '<tpl for="entitlements">',
      '<tpl if="editMode && id.length &gt; 0">',
        '<span style="float: right">',
          '<span style="padding: 12px" id="edit" class="smallFakeLink" onclick="Ext.getDom(\'editForm:editedProfileId\').value = \'{id}\'; Ext.getDom(\'editForm:editProfile\').click();">',
            '\[{parent.editI18n}\]',
          '</span>',
          '<span id="delete" style="padding: 12px" class="smallFakeLink" onclick="Ext.getDom(\'editForm:editedProfileId\').value = \'{id}\'; Ext.getDom(\'editForm:deleteProfile\').click();">',
            '\[{parent.deleteI18n}\]',
          '</span>',
        '</span>',
      '</tpl>',
                  ...
                  '<tpl for="rules">',
                    '<tr>',
                      '<tpl if="forRole.length &gt; 0">',
                        '<td>{forRole:htmlEncode}</td>',
                      '</tpl>',
                      '<td>{rule:htmlEncode}</td>',
                    '</tr>',
                  '</tpl>',
                '</tbody>',
              '</table>',
          ...
          '<tbody>',
            '<tpl for="entitlements">',
              '<tr>',
                ...
                '<td>{property}</td>',
                '<td>{displayValue:htmlEncode}',
                  '<tpl if="iiqElevatedAccess">',
                    '<img id="iiqElevatedAccess{[xindex]}" class="iiq-elevated-access-icon iiq-elevated-access-icon-image" style="cursor:pointer; padding:0px 7px 0px 7px" style="margin:0 2px 0 2px" height="14px" width="14px" alt="#{msgs.label_elevated_access}"/>',
                  '</tpl>',
                  '<tpl if="description.length &gt; 0">',
                    '<img id="imgHlpProfile{[xindex]}Info" class="iiq-elevated-access-icon iiq-elevated-access-icon-image" style="cursor:pointer; padding:0px 7px 0px 7px" alt="{description}"/>',
                  '</tpl>',
                '</td>',
              '</tr>',
            '</tpl>',
