...
Ext.define('SailPoint.Utils', {
    statics : {
        ...
        //ExtJs 5.1.0.2 include isEdge property support, until then use this function as a work around
        isEdge: function() {
            var edge = false,
                navigator = Ext.global.navigator;
            // IE 11 and Edge has Netscape as appName
        ...
        // Build a proper CSV string from the values in the given array
        arrayToCSV : function(theArray) {
            var csvStr = '';
            for (n = 0, len = theArray.length; n < len; n++) {
                var currValue = theArray[n] === null ? '' : theArray[n].toString();
                var result = currValue.replace(/"/g, '""');
                if (result.search(/("|,|\n)/g) >= 0)
                    result = '"' + result + '"';
                if (n > 0)
                    csvStr += ',';
        ...
        isSupportedBrowserJS: function() {
            if (Ext.isIE11 || this.isEdge()) {
                 ...
                 */
                if ((Ext.firefoxVersion >= 45 && Ext.firefoxVersion < 46)) {
                    return true;
                }
            }
            else if (Ext.isChrome) {
                if (Ext.chromeVersion >= 54) {
                    return true;
                }
            }
            else if (Ext.isSafari) {
                if (SailPoint.Platform.isMobile()) {
                    if (Ext.safariVersion === 10) {
                        return true;
                    }
                }
                else {
                    if (Ext.safariVersion >= 10 && Ext.safariVersion < 11) {
                        return true;
                    }
                }
            }
        ...
        /**
         * This function wraps all occurrences of the trademark symbol (®) in a given string with <sup> tags.
         * @param value The input string that may contain one or more trademark symbol (®)
         */
        wrapTrademarkWithSuperscript : function (value) {
            return value?.replace(/(&reg;|®)/g, '<sup>$1</sup>') ?? value;
        },
        ...
        validateNumberGreaterThanZero : function (textElement, errorLabel) {
            ...
            if (SailPoint.Utils.getValueFromElement (textElement, null) == null) {
                errorLabel.show();
                errorLabel.dom.innerHTML = "#{msgs.util_validate_enter_value}";
                valid = false;
            }
             else {
                ...
                if (val === -1) {
                    errorLabel.show();
                    errorLabel.dom.innerHTML = "#{msgs.util_validate_unable_to_parse}";
                    valid = false;
                }
                 else {
                    if (val <= 0) {
                        errorLabel.show();
                        errorLabel.dom.innerHTML = "#{msgs.util_validate_greater_than_zero}";
                        valid = false;
                    }
                }
            }
        ...
        /**
         * This function decode HTML entities in a string, So at first we will select
         * the HTML element then remove decode the HTML tag on innerHTML.
         * @param selector The selector string to get the HTML element
         */
        decodeHTMLElement: function(selector) {
            if (!selector) {
                return;
            }

            Ext.select(selector).each(function (element) {
                element.dom.textContent = Ext.String.htmlDecode(element.dom.textContent);
            });
        },
        ...
        tableTomahawkInputs : function() {
            ...
            Ext.select('input.tomahawkInputDate:first-child').each(function(firstInput) {
                ...
                if (parentSpan && parentSpan.is('span')) {
                    ...
                    // Clear existing content before adding new table.
                    parentSpan.update('');
        ...
        /*
            In a few places, we use the previous function (tableTomahawkInputs) to reformat the
            tomahawk date input so that each field is in a table cell. This works fine for date-only
            inputs, but messes up the formatting for time -- especially on the advanced search pages.
            This function is a variation that plays better for when both date & time is required, by
            putting time in its own table row, and preserving the ':' separator between hour & minute.
        */
        datetimeTomahawkTable: function() {
            //Select the first tomahawk input date of every set (for each t:inputDate)
            Ext.select('input.tomahawkInputDate:first-child').each(function(firstInput) {
                //This is the span containing all the tomahawk inputs as its children
                var parentSpan = firstInput.parent();
                if (parentSpan && parentSpan.is('span')) {
                    ...
                    var newTable = Ext.DomHelper.createDom({
                        tag: 'table',
                        cls: 'nopad tomahawkTable'});

                    //Insert a new row (-1 puts it at the end)
                    var newRow = newTable.insertRow(-1);
                    //... and a new cell in that row.
                    var newCell = newRow.insertCell(-1);
                    newCell.setAttribute('class','tomahawkTableDate');

                    //Query for all direct children of the span
                    Ext.each(parentSpan.query("> *"), function(child){
                        //Append each child input to the cell
                        newCell.appendChild(child);

                        var hourSuffix = ".hours";
                        var isHours = child.id.indexOf(hourSuffix, this.length - hourSuffix.length) !== -1;

                        // When we reach the calendar display button, insert a new row & cell.
                        // This allows us to show the time inputs together on their own line.
                        if (child.type == 'button') {
                            newRow = newTable.insertRow(-1);
                            newCell = newRow.insertCell(-1);
                            newCell.setAttribute('class','tomahawkTableTime');
                        } else if (isHours) {
                            // Add separator between hour & minute inputs
                            newCell.appendChild(Ext.DomHelper.createDom(' : '));
                        }
                    });

                    //All the children are moved into the table now, so put it under the span
                    // Clear existing content before adding table.
                    parentSpan.update('');
                    parentSpan.appendChild(newTable);
                }
        ...
        /*
         Escape for XSS security using OWASP recommendations
         & --> &amp;
         < --> &lt;
         > --> &gt;
         " --> &quot;
         ' --> &#x27;     &apos; not recommended as its not in HTML spec (See: section 24.4.1) it's is in the XML and XHTML specs.
         / --> &#x2F;     forward slash is included as it helps end an HTML entity
         */
        escapeForXss: function(str) {
            var encoded = "";
...
// Apply custom vTypes for validation
Ext.apply(Ext.form.field.VTypes, {
    ...
    daterangeText: '#{msgs.js_error_vtype_datarange}',
});
