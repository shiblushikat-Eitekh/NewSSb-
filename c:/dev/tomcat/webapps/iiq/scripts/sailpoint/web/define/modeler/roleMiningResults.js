...
SailPoint.roles.deleteResult = function(menuItem, event) {
    ...
    Ext.MessageBox.confirm('#{msgs.title_automated_mining}', Ext.String.format('#{msgs.are_you_sure_delete}', resultToDelete), function(buttonId) {
        if (buttonId == 'yes') {
            Ext.getDom('itRoleMiningDeleteResultForm:itRoleMiningResultToDelete').value = resultToDelete;
            Ext.getDom('itRoleMiningDeleteResultForm:deleteITRoleMiningTaskResult').click();
        } 
    });
};
...
SailPoint.roles.getMiningResultsGrid = function() {
    var resultsColumnConfig = Ext.JSON.decode(Ext.getDom('miningResultsGridConfig').value);
...
// headerContentDivs[i].style['overflow'] = 'visible';
            ...
            Ext.each(view.getNodes(), SailPoint.roles.MiningResultViewer.styleRow);
        });
...
Ext.define('SailPoint.roles.MiningResultViewer', {
    ...
    statics: {
        styleRow: function(row) {
            ...
            Ext.each(Ext.query('td', row), function(cell) {
    ...
    var groupSummaryHandler = function(item, eventObj) {
        ...
        if (!groupSummaryWindow) {
            groupSummaryWindow = new Ext.Window({
               ...
               html: '<div id="itRoleMiningGroupSummaryEntitlements"></div>',
               closeAction: 'hide'
            });
            ...
            groupSummaryWindow.sizeToEntitlements = function() {
                var entitlementTable = Ext.DomQuery.selectNode('table[class*=entitlementsBodyTable]', Ext.get('itRoleMiningGroupSummaryEntitlements'));
                var entitlementTableWrapper = Ext.get(Ext.DomQuery.selectNode('div[class*=entitlementsBodyTableWrapper]', Ext.get('itRoleMiningGroupSummaryEntitlements')));
            ...
            groupSummaryWindow.setTitle(Ext.String.format('#{msgs.it_role_mining_entitlements_group}', Ext.String.htmlEncode(record.get('identifier'))));
        }
...
SailPoint.roles.displayRoleCreationDialog = function (entitlementInfo) {
    ...
    if (creationDialog) {
        Ext.getDom('createFromItRoleMiningForm:createRoleFromITMiningName').value = '';
        Ext.getDom('createFromItRoleMiningForm:createRoleFromITRoleMiningDescription').value = '';
...
Ext.define('SailPoint.roles.RoleCreationDialog', {
                ...
                handler: function() {
                    ...
                    if (creationDialog.validate()) {
                        ...
                        if(roleDescriptionCmp) {
                            Ext.getDom('createFromItRoleMiningForm:createRoleFromITRoleMiningDescription').value = roleDescriptionCmp.getCleanValue();
                        }
                        Ext.getDom('createFromItRoleMiningForm:createRoleFromITRoleMining').click();
                    }
                }
            },{
    ...
    initScopeSuggest: function() {
        ...
        if (!scopeSuggest) {
            scopeSuggest = new SailPoint.ScopeSuggest({
                ...
                value: Ext.getDom('createRoleFromITMiningScope').value,
                width: 200,
    ...
    initOwnerSuggest: function() {
        ...
        if (!ownerSuggest) {
            ownerSuggest = new SailPoint.IdentitySuggest({
                ...
                binding: Ext.getDom('createFromItRoleMiningForm:createRoleFromITMiningOwner'),
                allowBlank: false,
                rawValue: Ext.getDom('createRoleFromITMiningDefaultOwner').innerHTML,
    ...
    initContainerRoleSuggest: function() {
        ...
        if (!containerRoleSuggest) {
            new SailPoint.BaseSuggest({
                ...
                binding: Ext.getDom('createFromItRoleMiningForm:createRoleFromITMiningContainerRole'),
                allowBlank: true,
    ...
    initInheritedRoleSuggest: function() {
        ...
        if (!inheritedRolesMultiSuggest) {
            ...
            inheritedRolesMultiSuggest.selectedStore.on('add', function() {Ext.getDom('createFromItRoleMiningForm:updateEntitlementsForInheritedRoles').click();}, this);
            inheritedRolesMultiSuggest.selectedStore.on('clear', function() {Ext.getDom('createFromItRoleMiningForm:updateEntitlementsForInheritedRoles').click();}, this);
            inheritedRolesMultiSuggest.selectedStore.on('remove', function() {Ext.getDom('createFromItRoleMiningForm:updateEntitlementsForInheritedRoles').click();}, this);
        }
    },
    
    resetDescriptions: function() {
        var allowRoleLocalization = Ext.getDom("allowRoleLocalization").innerHTML;
        ...
        if(!descriptionCmp && allowRoleLocalization == "true") {
            var descriptionEditor = new SailPoint.MultiLanguageHtmlEditor({
                ...
                value: '',
                defaultLocale: Ext.getDom("createRoleFromITRoleMiningDescrDefaultLocale").innerHTML
            });
    ...
    displayCreateResults: function() {
        ...
        this.hide();
        results = Ext.JSON.decode(Ext.getDom('ITRoleMiningRoleCreationResults').innerHTML);
        if (results.success) {
            Ext.MessageBox.show({
                title:'#{msgs.title_automated_mining}',
                msg: results.message,
    ...
    validate: function() {
        var name = Ext.getDom('createFromItRoleMiningForm:createRoleFromITMiningName').value;
        var includedEntitlements = Ext.getCmp('directEntitlementsSelector').entitlementsObj;
        var isValid = true;
        
        if (!name || name.trim().length == 0) {
            Ext.getDom('itRoleMiningCreateRoleNameError').style['display'] = '';
            Ext.MessageBox.show({
                title:'#{msgs.err_dialog_title}',
