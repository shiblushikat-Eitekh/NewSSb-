        ...
        bbar: [{
            ...
            handler: function() {
                ...
                if (runTheSearch) {
                    ...
                    // upon completion
                    Ext.getDom('processInstrumentationSearchForm:processInstrumentationSearchBtn').click();
                }
            }
        }, {
...
SailPoint.ProcessInstrumentation.Search.validate = function() {
    ...
    Validator.validateGreaterThanOrEqual('processInstrumentationSearchForm:executionTimeThreshold', 0, Ext.String.format('#{msgs.err_number_less_than_zero}', Ext.getDom('processInstrumentationSearchForm:executionTimeThreshold').value), false);
    errors = Validator.getErrors();
    if (errors.length > 0) {
        Validator.displayErrors(Ext.get('ProcessInstrumentationError'));
        isValid = false;
    } else {
...
SailPoint.ProcessInstrumentation.Search.afterRunSearchSubmit = function() {
    ...
    resultsPanel.update({timeUnits:Ext.getDom('processInstrumentationSearchForm:executionTimeThresholdUnits').value});
};
...
SailPoint.ProcessInstrumentation.Search.initSearchContents = function() {
    ...
    participantMultiSuggest = new SailPoint.MultiSuggest({
        ...
        jsonData: JSON.parse(Ext.getDom('processParticipantMultiSuggestData').innerHTML),
    ...
    processNameSuggest = new SailPoint.BaseSuggest({
        ...
        value: Ext.getDom('processNameInitialVal').innerHTML,
        valueField: 'id',
    ...
    enableStartDate = Ext.getDom('processInstrumentationSearchForm:processInstrumentationEnableStartDateInput').value == 'true';
    Ext.getDom('processInstrumentationEnableStartDate').checked = enableStartDate;
    toggleDisplay(Ext.getDom('processInstrumentationStartDateDiv'), !enableStartDate);
    
    enableEndDate = Ext.getDom('processInstrumentationSearchForm:processInstrumentationEnableEndDateInput').value == 'true';
    Ext.getDom('processInstrumentationEnableEndDate').checked = enableEndDate;
    toggleDisplay(Ext.getDom('processInstrumentationEndDateDiv'), !enableEndDate);


    if (!contentPanel.originalStart) {
        year = Ext.getDom(startInput + '.year').value;
        month = Ext.getDom(startInput + '.month').value;
        day = Ext.getDom(startInput + '.day').value;
        contentPanel.originalStart = new Date(year, month, day);
    }
    
    if (!contentPanel.originalEnd) {
        year = Ext.getDom(endInput + '.year').value;
        month = Ext.getDom(endInput + '.month').value;
        day = Ext.getDom(endInput + '.day').value;
        contentPanel.originalEnd = new Date(year, month, day);
    }
    
    SailPoint.ProcessInstrumentation.Search.styleSearchPanels();
    SailPoint.Analyze.initializeSubmit('processInstrumentationSearchForm', 'runInstrumentationSearchBtn');
...
SailPoint.ProcessInstrumentation.Search.styleSearchPanels = function() {
    ...
    buildTooltips(Ext.getDom('processInstrumentationSearchContents'));
...
//        var resultsContent = Ext.get(Ext.DomQuery.selectNode('div[class*=spBackground]', Ext.get('roleResultsForm')));
...
SailPoint.ProcessInstrumentation.Search.clearSearchFields = function() {
    ...
    if (Ext.getDom('processInstrumentationEnableStartDate').checked) {
        toggleDisplay(Ext.getDom('processInstrumentationStartDateDiv'), true);
        Ext.getDom('processInstrumentationEnableStartDate').checked = false;
        Ext.getDom('processInstrumentationSearchForm:processInstrumentationEnableStartDateInput').value = false;
    }

    if (Ext.getDom('processInstrumentationEnableEndDate').checked) {
        toggleDisplay(Ext.getDom('processInstrumentationEndDateDiv'), true);
        Ext.getDom('processInstrumentationEnableEndDate').checked = false;
        Ext.getDom('processInstrumentationSearchForm:processInstrumentationEnableEndDateInput').value = false;
    }

    Ext.getDom('processInstrumentationSearchForm:executionTimeThreshold').value = '0.0';
    ...
    Ext.getDom('processName').value = '';
    Ext.getCmp('processParticpantMultiSuggestCmp').clear();
    Ext.getDom('processParticipantSuggest').value = '';
...
SailPoint.ProcessInstrumentation.Search.displayDetailsMenu = function(eventObj, record) {
    ...
    menu.currentParams = {
        ...
        timeUnits:Ext.getDom('processInstrumentationSearchForm:executionTimeThresholdUnits').value
    };
...
Ext.define('SailPoint.ProcessInstrumentation.Search.DetailsMenu', {
    ...
    items: [{
        ...
        handler: function(item, eventObj) {
            Ext.getDom('processMetricsEditForm:processToEdit').value = item.parentMenu.currentParams.process;
            Ext.getDom('processMetricsEditForm:editProcess').click();
        }        
    }]
});
