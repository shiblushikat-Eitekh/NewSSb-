...
function populateEntitlementCell( cell, violation ) {
    ...
    if(violation.iiqElevatedAccess){
...
function getViolationDisplayableValue(violation) {
    // IIQFW-2: Encode entitlement name to avoid HTML injection
    return Ext.String.htmlEncode(
        (violation.displayValue) ? violation.displayValue : violation.name + ' ' + violation.value);
}
