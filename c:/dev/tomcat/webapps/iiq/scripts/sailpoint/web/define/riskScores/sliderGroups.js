...
Control.SliderGroup = function(aSliderArray, total) {
...
Control.SliderGroup.prototype = {
    ...
    getSlider: function(sliderId) {
        for (var i = 0; i < this.sliderArray.length; ++i) {
            ...
            if (sliderId === '0' && this.sliderArray[i].id !== '0') {
                this.sliderArray[i].id = sliderId;
            }
    ...
    _getNumUnlockedSliders: function() {
        ...
        for (var i = 0; i < this.lock.length - 1; ++i) {
            if (!this.lock[i]) {
                numUnlockedSliders++;
            }
        }
