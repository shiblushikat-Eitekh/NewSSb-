      ...
      tbar: [
       {
         ...
         
         {
           ...
           handler:function(){
             ...
             schedulesGrid.getStore().load({params:{start:0,limit:25}});
             schedulesGrid.getPagingToolbar().moveFirst();
           }
         },
         {
           xtype: 'button',
      ...
      tbar: [
       ...
       
       {
         ...
         handler:function(){
           ...
           resultsGrid.getStore().load({params:{start:0, limit:25}});
           resultsGrid.getPagingToolbar().moveFirst();
         }
       },
       {
         xtype:'button',
...
SailPoint.Monitor.Grid.Tasks.resultsContextMenu = function(gridView, record, HTMLitem, index, e, eOpts) {
  ...
  var canRestart = record.get('canRestart');
  ...
  if (SailPoint.userHasTaskWriteAccess) {
	  if (statusId=='-1' || statusId == -1) {
	    contextMenu.add(new Ext.menu.Item({text: '#{msgs.menu_terminate}', handler: terminateResultPrompt, iconCls: 'terminateBtn'}));
	      ...
	      if (statusId=='3' || statusId == 3 || statusId=='0' || statusId==0) {
	          if (canRestart) {
                  contextMenu.add(
                      new Ext.menu.Separator(),
                      new Ext.menu.Item({
                          text: '#{msgs.menu_restart}',
                          handler: restartResultPrompt,
                          iconCls: 'restartBtn'
                      })
                  );
              }
...
SailPoint.Monitor.Grid.Tasks.createEventProcessingDetailsGrid = function(renderToId, taskResultId) {
	  var store = SailPoint.Store.createRestStore({
	      fields: ['eventBundle','identitiesToProcess','identitiesSkipped', 'identitiesFailed', 'identitiesSucceeded', 'eventStatus'],
	      root: 'objects',
	      totalProperty: 'count',
	      url: SailPoint.getRelativeUrl('/rest/taskResults/' + taskResultId + '/attributes/eventsProcessingDetails'),
	      method : 'GET',
	      autoLoad: true,
	      remoteSort: false
	  });
	  ...
	  //columns     
	  var columns = [{
	      header: '#{msgs.task_out_event_bundle}',
	      dataIndex: 'eventBundle',
	      sortable: false, 
	      hideable: false
	  },{
	      header: '#{msgs.task_out_event_identities_to_process}',
	      dataIndex: 'identitiesToProcess',
	      sortable: false, 
	      hideable: false
	  },{
	      header: '#{msgs.task_out_event_identities_skipped}',
	      dataIndex: 'identitiesSkipped',
	      sortable: false, 
	      hideable: false
	  },{
	      header: '#{msgs.task_out_event_identities_failed}',
	      dataIndex: 'identitiesFailed',
	      sortable: false, 
	      hideable: false
	  },{
	      header: '#{msgs.task_out_event_identities_succeeded}',
	      dataIndex: 'identitiesSucceeded',
	      sortable: false, 
	      hideable: false
	  },{
	      header: '#{msgs.task_out_event_status}',
	  ...
	  var eventDetailssGrid = Ext.create('SailPoint.grid.PagingGrid', {
	        store: store,
	        id: 'eventProcessingDetailsGrid',
	        layout:'fit',
	        columns:columns,
	        pageSize: 15
	  });
	    ...
	    var ct = Ext.create('Ext.panel.Panel', {
	        items : [ eventDetailssGrid ],
	        title:'#{msgs.task_out_events_processing_details}',
	        renderTo: renderToId,
	        collapsible: true,
	        collapsed: false,
	        layout:'fit',
