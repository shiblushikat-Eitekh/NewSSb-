...
SailPoint.IdentityRequest.Search.getIdentityRequestSearchPanel = function(config) {
    ...
    var searchContents = Ext.create('Ext.panel.Panel', {
        ...
        dockedItems: [{
            ...
            items: [{
                id: 'advancedIdentityRequestSearchNavBtn',
                text: '#{msgs.advanced_search}',
                scale : 'medium',
                handler: SailPoint.IdentityRequest.Search.displayAdvancedSearch,
                cls: 'x-btn-text-icon'
            }]
        }],
        bbar: [{
    ...
    var advancedSearchContents = Ext.create('Ext.panel.Panel', {
        id: 'advancedIdentityRequestSearchContents',
        layout: 'fit',
        contentEl: 'advancedIdentityRequestSearchContentsDiv',
        border: false,
        autoScroll: true,
        dockedItems: [{
            xtype: 'toolbar',
            ...
            cls: 'advancedSearchToolbar',
            items: [{
                id: 'identityRequestSearchNavBtn',
                text: '#{msgs.identity_request_search}',
                scale : 'medium',
                handler: SailPoint.IdentityRequest.Search.displaySearchContents,
                cls: 'x-btn-text-icon'
            }]
        }],
        bbar: [{
            id: 'preAdvancedIdentityRequestSearchBtn',
            text: '#{msgs.button_run_search}',
            cls: 'primaryBtn',
            handler: function() {
                var searchPanel = Ext.getCmp('identityRequestSearchPanel');
                searchPanel.validationErrors = SailPoint.Analyze.validateSearch('advancedIdentityRequestSearchForm', 'advancedIdentityRequest');
            }
        }, {
            id: 'advancedIdentityRequestClearBtn',
            text: '#{msgs.button_clear_search}',
            handler: function() {
                Ext.getDom('advancedIdentityRequestSearchForm:resetBtn').click()
            }
        }],
        loader: {
            url: SailPoint.getRelativeUrl('/analyze/advanced/advancedSearchContents.jsf'),
            params: {searchType: 'AdvancedIdentityRequest'},
            discardUrl: false,
            callback: SailPoint.IdentityRequest.Search.finishAdvancedSearchInit,
            nocache: false,
            text: '#{msgs.loading_data}',
            timeout: 30,
            scripts: true
    ...
    var advancedResultsContents = SailPoint.Search.getResultsGrid({
        id: 'advancedIdentityRequestSearchResultsGrid',
        type: 'advancedIdentityRequest',
        stateful: true,
        stateId: SailPoint.Analyze.gridStateIds.get('AdvancedIdentityRequest'),
        url: SailPoint.getRelativeUrl('/analyze/identityRequest/advancedIdentityRequestDataSource.json'),
        handleClick: SailPoint.IdentityRequest.Search.handleClick,
        pageSize: SailPoint.Analyze.defaultResultsPageSize,
        sorters : [{ property: 'name', direction: 'ASC' }, {property: 'id', direction: 'ASC'}],
        optionsPlugin: SailPoint.Search.getOptionsPlugin({
            searchType: 'AdvancedIdentityRequest',
            cardPanelId: 'identityRequestSearchPanel',
            searchPanelId: 'advancedIdentityRequestSearchContents',
            applySearchPanelStyles: function() {
                Ext.getDom('advancedSearchWrapperTbl').style['width'] = '95%';
                Ext.getCmp('advancedIdentityRequestSearchContents').doLayout();
            },
            options: [
                ['saveOrUpdate', '#{msgs.save_search}'],
                ['saveAsReport', '#{msgs.save_search_as_report}']
...
SailPoint.IdentityRequest.Search.displayAdvancedSearchResults = function() {
    var searchPanel = Ext.getCmp('identityRequestSearchPanel');
    searchPanel.getLayout().setActiveItem('advancedIdentityRequestSearchResultsGridWrapper');
    searchPanel.doLayout();
    if (Ext.isGecko) {
        Ext.getCmp('identityRequestSearchResultsGrid').getView().refresh();
    }
...
SailPoint.IdentityRequest.Search.displayAdvancedSearch = function() {
    SailPoint.Search.clearForRefineSearch('advancedIdentityRequestSearchResultsGrid');

    var searchPanel = Ext.getCmp('identityRequestSearchPanel');
    searchPanel.getLayout().setActiveItem('advancedIdentityRequestSearchContents');

    if (!searchPanel.hasDisplayFields) {
        SailPoint.Analyze.SearchDisplayFields.initDisplayFields('advancedIdentityRequest');
        searchPanel.hasDisplayFields = true;
    }
    Ext.getCmp('advancedIdentityRequestDisplayFieldsPanel').doLayout();
...
SailPoint.IdentityRequest.Search.finishAdvancedSearchInit = function() {
    advIdentityRequestSearchFiltersPage = FiltersPage.instance('div.advIdentityRequestSearchspTabledAjaxContent', 'advIdentityRequestSearchfilterBeanListTbl', 'advancedIdentityRequestSearchForm', 'advIdentityRequestSearch');
    advIdentityRequestSearchFiltersPage.initPage();
    buildTooltips(Ext.getDom('advancedIdentityRequestAttributes'));
    buildTooltips(Ext.getDom('filterCriteria'));
    SailPoint.BaseGrid.initGrid(SailPoint.getRelativeUrl('/analyze/identityRequest/advancedIdentityRequestDataSource.json'), 'advancedIdentityRequestSearchResultsGrid', 13, true);
    Ext.getDom('advancedSearchWrapperTbl').style['width'] = '95%';
    Ext.getCmp('advancedIdentityRequestSearchContents').doLayout();
};
...
SailPoint.IdentityRequest.Search.clearAdvancedIdentityRequestSearchFields = function() {
    SailPoint.Analyze.resetFieldsToDisplay('advancedIdentityRequest');
    advIdentityRequestSearchFiltersPage.clearSearchFields();
};
...
SailPoint.Analyze.IdentityRequest.initializeAttributes = function() {
  ...
  requestorSuggest = new SailPoint.IdentitySuggest({
    ...
    baseParams: {context: 'IncludeWorkGroups'},
    width: 200,
