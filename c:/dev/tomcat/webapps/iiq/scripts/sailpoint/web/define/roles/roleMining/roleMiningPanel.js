...
Ext.define('SailPoint.Role.ITRoleMiningPanel', {
	extend : 'SailPoint.Role.BaseRoleMiningPanel',
                ...
                handler: function() {
                    ...
                    if( validated ) {
                        SailPoint.roles.saveTemplatePrompt({
                            ...
                            templateName: Ext.getDom('scheduleItRoleMiningForm:itRoleMiningTemplateName').value,
                            originalTemplateName: Ext.getDom('scheduleItRoleMiningForm:itRoleMiningOriginalTemplateName').value,
                            saveFromExistingHandler: function() {
                                if(Ext.getCmp('itWindowTemplateSaveNew').checked) {
                                    Ext.getDom('scheduleItRoleMiningForm:itRoleMiningTemplateId').value = '';
                                    Ext.getDom('scheduleItRoleMiningForm:itRoleMiningTemplateName').value = Ext.getCmp('itWindowTemplateName').getValue();
                                }
                                Ext.getCmp('itMiningTemplateSaveWindow').hide();
                                Ext.getDom('scheduleItRoleMiningForm:saveAsNew').click();
                            },
                            saveFromScratchHandler: function(btn, text) {
                                Ext.getDom('scheduleItRoleMiningForm:itRoleMiningTemplateId').value = '';
                                Ext.getDom('scheduleItRoleMiningForm:itRoleMiningTemplateName').value = text;
                                Ext.getDom('scheduleItRoleMiningForm:saveAsNew').click();
                            }
                        });
                    } else {
                        ...
                        SailPoint.roles.saveTemplatePrompt({
                            ...
                            saveFromExistingHandler: function() {
                                if(Ext.getCmp('itSaveAndExecWindowTemplateSaveNew').checked) {
                                    Ext.getDom('scheduleItRoleMiningForm:itRoleMiningTemplateId').value = '';
                                    Ext.getDom('scheduleItRoleMiningForm:itRoleMiningTemplateName').value = Ext.getCmp('itSaveAndExecWindowTemplateName').getValue();
                                }
                                Ext.getCmp('itSaveAndExecMiningTemplateSaveWindow').hide();
                                Ext.getDom('scheduleItRoleMiningForm:saveAndExecuteButton').click();
                            },
                            saveFromScratchHandler: function(btn, text) {
                                Ext.getDom('scheduleItRoleMiningForm:itRoleMiningTemplateId').value = '';
                                Ext.getDom('scheduleItRoleMiningForm:itRoleMiningTemplateName').value = text;
    ...
    getIpopPanelInfo: function() {
        var info = {
            ipopSelectId: 'scheduleItRoleMiningForm:ipopName',
            ipopDescriptionDivId: 'itRoleMiningIpopDescription',
            ipopSizeDivId: 'itRoleMiningIpopSize',
            ipopAppsDivId: 'itRoleMiningIpopApps'
        };
        return info;
    },
    ...
    validate: function() {
        ...
        /* Validate name */
        var templateNameField = Ext.getDom('scheduleItRoleMiningForm:itRoleMiningTemplateName');
        var templateNameErrorField = Ext.getDom('itRoleMiningTemplateNameError');
        ...
        /* Validate owner */
        var templateOwnerField = Ext.getDom( 'scheduleItRoleMiningForm:itRoleMiningOwner' );
        var templateOwnerErrorField = Ext.getDom( 'itRoleMiningTemplateNoOwnerError' );
        ...
        var applicationsToMineField = Ext.getDom( 'scheduleItRoleMiningForm:itRoleMiningAppNameSuggest' );
        var applicationsToMineErrorField = Ext.getDom( 'itRoleMiningTemplateNoApplicationsError' );
    ...
    showSearchFilterOptions: function(optionToShow) {
        if (optionToShow == 'searchByIpop') {
            Ext.getDom('identityAttributesRow').style['display'] = 'none';
            Ext.getDom('ipopSelectorRow').style['display'] = '';
        } else {
            Ext.getDom('identityAttributesRow').style['display'] = '';
            Ext.getDom('ipopSelectorRow').style['display'] = 'none';
        }
    },
    ...
    initMiningAppSuggest: function() {
        ...
        if (miningAppNameMultiSuggest) {
            miningAppNameMultiSuggest.selectedStore.loadData(Ext.decode(Ext.getDom('itRoleMiningAppNameMultiSuggestData').innerHTML));
        } else {
            miningAppNameMultiSuggest = new SailPoint.MultiSuggest({
                ...
                jsonData: Ext.decode(Ext.getDom('itRoleMiningAppNameMultiSuggestData').innerHTML),
    ...
    initMiningOwnerSuggest: function() {
        ...
        if (!miningOwnerSuggest) {
            miningOwnerSuggest = new SailPoint.IdentitySuggest({
                ...
                binding: Ext.getDom('scheduleItRoleMiningForm:itRoleMiningOwner'),
                allowBlank: false,
        ...
        miningOwnerSuggest.setRawValue(Ext.getDom( 'scheduleItRoleMiningForm:itRoleMiningOwnerName' ).value);
    },
    ...
    initExcludedEntitlementsSuggest: function() {
        ...
        if (excludedEntitlementsSuggest) {
            excludedEntitlementsSuggest.setValue(Ext.getDom( 'itRoleMiningExcludedEntitlementsSuggest' ).value);
        } else {            
                    ...
                    tpl: new Ext.XTemplate(
                        '<tpl for=".">',
                            '<div class="baseSearch x-boundlist-item">',
                                ...
                                '<div class="indentedColumn">{displayName:htmlEncode}</div>',
                            '</div>',
                        '</tpl>'
                    ), 
    ...
    addApplication: function(store, appsThatWereAdded, indexWhereTheyWereAdded) {
        ...
        Ext.getCmp('itRoleMiningExcludedEntitlementsSuggestCmp').getStore().load({
            ...
            callback: Ext.getCmp('itRoleMiningPanel').afterPopulationUpdate
        });
    },
    ...
    clearEntitlements: function(applicationStore, options) {
        ...
        if (!removedApplicationRecord) {
            ...
            if (!Ext.getDom('scheduleItRoleMiningForm:itRoleMiningExcludedEntitlements').value || Ext.getDom('scheduleItRoleMiningForm:itRoleMiningExcludedEntitlements').value.length == 0) {
                excludedEntitlementsObj = {entitlements: []};
            } else {
                excludedEntitlementsObj = Ext.JSON.decode(Ext.getDom('scheduleItRoleMiningForm:itRoleMiningExcludedEntitlements').value);
            }
        ...
        Ext.getDom('scheduleItRoleMiningForm:itRoleMiningExcludedEntitlements').value = Ext.JSON.encode(excludedEntitlementsObj);
        this.updateEntitlementsTable();
        this.afterPopulationUpdate();
    },
    
    clearMiningParams: function() {
        var defaults = Ext.JSON.decode(Ext.getDom('itRoleMiningDefaults').innerHTML);
        Ext.getDom('scheduleItRoleMiningForm:itRoleMiningTemplateId').value = '';
        Ext.getDom('scheduleItRoleMiningForm:itRoleMiningTemplateName').value = '';
        if (Ext.getDom('scheduleItRoleMiningForm:itRoleMiningOwner').value != defaults.ownerId) {
            Ext.getDom('scheduleItRoleMiningForm:itRoleMiningOwner').value = defaults.ownerId;
        ...
        Ext.getDom('scheduleItRoleMiningForm:minIdentities').value = defaults.minIdentities;
        Ext.getDom('scheduleItRoleMiningForm:minEntitlements').value = defaults.minEntitlements;
        Ext.getDom('scheduleItRoleMiningForm:maxCandidates').value = defaults.maxCandidateRoles;
        this.afterPopulationUpdate();
        /* Reset Error fields */
        var templateOwnerErrorField = Ext.getDom( 'itRoleMiningTemplateNoOwnerError' );
        ...
        var itRoleMiningSearchInfo = Ext.get('itRoleMiningSearchInfo');
    ...
    resetAttributeSelector: function() {
        var inputs = Ext.DomQuery.select('input[type=radio]', Ext.getDom('scheduleItRoleMiningForm:attributeOrIpopSelector'));
    ...
    resetAttributesTable: function() {
        var inputs = Ext.DomQuery.select('input[type=text]', Ext.getDom('itRoleMiningSearchAttributesTable'));
        var options = Ext.DomQuery.select('option', Ext.getDom('itRoleMiningSearchAttributesTable'));
        var i;
    ...
    updatePopulationOnKeypress: function() {
        ...
        this.delayedTask.delay(delay, this.afterPopulationUpdate, this);
    },
    ...
    afterPopulationUpdate: function(currentIpopInfo) {
        ...
        if (multiSuggest 
                && currentIpopInfo
                && currentIpopInfo.applications 
                && currentIpopInfo.applicationIds) {
    ...
    loadContent: function(forceRerender, callback) {
        ...
        if (callback) {
            afterLoadFn = function() {
                ...
                contentPanel.initMiningIpopSelector(contentPanel.getIpopPanelInfo());
                // contentPanel.updateEntitlementsTable(); -- The callback should handle this
                contentPanel.showSearchFilterOptions( Ext.getDom( 'scheduleItRoleMiningForm:populationFilterType' ).value );
                if( Ext.getDom( 'scheduleItRoleMiningForm:itRoleMiningTemplateId' ).value == '' ) {
    ...
    updateEntitlementsTable: function() {
        var entitlementsToInclude = Ext.JSON.decode(Ext.getDom('scheduleItRoleMiningForm:itRoleMiningExcludedEntitlements').value);
        ...
        buildTooltips(Ext.getDom('itRoleMiningEntitlementsToInclude'));
    ...
    viewLastTaskResult: function() {
        var name = Ext.getDom( 'scheduleItRoleMiningForm:itRoleMiningTemplateName' ).value;
        var templateId = Ext.getDom('scheduleItRoleMiningForm:itRoleMiningTemplateId').value;
            ...
            callback: function(options, success, response) {
                ...
                if (responseObj.isViewable) {
                    ...
                    Ext.MessageBox.show({
                        ...
                        msg: Ext.String.htmlEncode(responseObj.errorMsg),
                        buttons: Ext.MessageBox.OK
                    });
                }
            }
    ...
    displayResultsMessage : function() {
        var launchResults = Ext.JSON.decode(Ext.getDom('itMiningLaunchResults').innerHTML);
