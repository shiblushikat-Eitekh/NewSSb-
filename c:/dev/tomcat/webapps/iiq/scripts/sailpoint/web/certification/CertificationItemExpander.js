    ...
    showContent : function(row, record, type){
        ...
        switch(type) {
            ...
            case "history":
                ...
                var identity = record.data['IIQ_identity'] ? record.data['IIQ_identity'].id : record.data['parent-targetId'];
                var workItemId = record.get('IIQ_decisionChoices') ? record.get('IIQ_decisionChoices').workItemId : null;
                panel = SailPoint.IdentityHistoryPanel.getPanel(SailPoint.Utils.getRecordId(record), identity, targetEl, workItemId);
...
SailPoint.certification.AccountExpando = function(row, record, targetEl){
    ...
    var rid = SailPoint.Utils.getRecordId(record);
    var url = '/identity/linkDetails.jsf?nonEntitlements=false&certItem=' + rid + '&refererType=certificationItem&refererId=' + rid;
...
SailPoint.certification.RoleExpando = function(row, record, targetEl){
    ...
    var certItemId = SailPoint.Utils.getRecordId(record);
    return SailPoint.RoleDetailPanel.toggle(roleId, identityId, isAssignedRole, targetEl, certItemId, null, roleAssignmentId, roleType, 'certificationItem', certItemId);
};
...
SailPoint.certification.IdentityExpando = function(row, record, targetEl){
    ...
    tabs.items.get(1).on('activate', function(thePanel) {
        //IIQETN-5149 :- Appending workItemId to the URL if different than null
        var url = '/manage/certification/identityLinksPanel.jsf?itemId=' + itemId;
        if (workItemId) {
            url += "&workItemId=" + workItemId;
        }
        if(!thePanel.loader) {
...
SailPoint.certification.LinkExpando = function(row, record, targetEl){
    ...
    var url = '/identity/linkDetails.jsf?certItem=' + rid + '&refererType=certificationItem&refererId=' + rid;
    if (Page.showEntitlementDescriptions != undefined){
        url += '&showDesc=' + Page.showEntitlementDescriptions;
    }
